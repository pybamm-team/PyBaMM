...
class TestCasadiSolver(unittest.TestCase):
    ...
    def test_model_solver(self):
        ...
        np.testing.assert_array_almost_equal(
            solution["var"].data, np.exp(0.1 * solution.t), decimal=5
        )
    ...
    def test_without_grid(self):
        ...
        model.events = [pybamm.Event("an event", var + 1)]
        model.variables = {"var": var, "var2": var2}
        ...
        np.testing.assert_array_equal(solution.t, t_eval)
        np.testing.assert_array_almost_equal(
            solution["var"].data, np.exp(0.1 * t_eval), decimal=5
        )
        np.testing.assert_array_almost_equal(
            solution["var2"].data, np.ones_like(t_eval), decimal=5
        )
    ...
    def test_model_solver_events(self):
        ...
        solvers = {
            ...
            pybamm.CasadiSolver(mode="safe", rtol=1e-8, atol=1e-8),
            # Try dt_max=0 to enforce using all timesteps
            pybamm.CasadiSolver(mode="safe", rtol=1e-8, atol=1e-8, dt_max=0),
            # Solve using "fast with events" mode
            pybamm.CasadiSolver(mode="fast with events", rtol=1e-8, atol=1e-8),
        }
        ...
        for solver in solvers:
            with self.subTest(solver=solver):
                solution = solver.solve(model, t_eval)
                np.testing.assert_array_less(solution["var1"].data[:-1], 1.5)
                np.testing.assert_array_less(solution["var1"].data[:-1], 2.5)
                np.testing.assert_equal(solution.t_event[0], solution.t[-1])
                np.testing.assert_array_equal(
                    solution.y_event[:, 0], solution._all_ys[-1].flatten()
                )
                np.testing.assert_array_almost_equal(
                    solution["var1"].data[0], np.exp(0.1 * solution.t), decimal=5
                )
                np.testing.assert_array_almost_equal(
                    solution["var2"].data[0], 2 * np.exp(0.1 * solution.t), decimal=5
                )
        ...
        solver = pybamm.CasadiSolver(rtol=1e-8, atol=1e-8)
        solution = solver.solve(model, t_eval)
        np.testing.assert_array_less(solution["var"].data, 1.02 + 1e-10)
        np.testing.assert_array_almost_equal(solution["var"].data[-1], 1.02, decimal=2)
    ...
    def test_model_step(self):
        ...
        t_eval = step_sol.t
        solution = solver.solve(model, t_eval)
        np.testing.assert_array_almost_equal(solution["var"].data, step_sol["var"].data)
    ...
    def test_model_step_with_input(self):
        ...
        np.testing.assert_allclose(step_sol["var"].data, np.exp(0.1 * step_sol.t))
    ...
    def test_model_step_events(self):
        ...
        while time < end_time:
            ...
            time += dt
        np.testing.assert_array_less(step_solution["var1"].data[:-1], 1.5)
        np.testing.assert_array_less(step_solution["var1"].data[:-1], 2.5)
        np.testing.assert_equal(step_solution.t_event[0], step_solution.t[-1])
        np.testing.assert_array_equal(
            step_solution.y_event[:, 0], step_solution._all_ys[-1].flatten()
        )
        np.testing.assert_array_almost_equal(
            step_solution["var1"].data, np.exp(0.1 * step_solution.t), decimal=5
        )
        np.testing.assert_array_almost_equal(
            step_solution["var2"].data, 2 * np.exp(0.1 * step_solution.t), decimal=4
        )
    ...
    def test_model_solver_with_inputs(self):
        ...
        np.testing.assert_allclose(
            solution["var"].data[0], np.exp(-0.1 * solution.t), rtol=1e-04
        )
        ...
        for rate in [0.1, 1.1]:
            ...
            solution = solver.solve(model, t_eval, inputs={"rate": rate})
            self.assertLess(len(solution.t), len(t_eval))
            np.testing.assert_allclose(
                solution["var"].data[0], np.exp(-rate * solution.t), rtol=1e-04
            )
    ...
    def test_model_solver_dae_inputs_in_initial_conditions(self):
        ...
        np.testing.assert_array_almost_equal(
            solution["var1"].data, 0.1 * np.exp(-solution.t), decimal=5
        )
        np.testing.assert_array_almost_equal(
            solution["var2"].data, 0.1 * np.exp(-solution.t), decimal=5
        )
        ...
        np.testing.assert_array_almost_equal(
            solution["var1"].data, 1 * np.exp(-0.1 * solution.t), decimal=5
        )
        np.testing.assert_array_almost_equal(
            solution["var2"].data, 1 * np.exp(-0.1 * solution.t), decimal=5
        )
    ...
    def test_model_solver_with_non_identity_mass(self):
        ...
        solution = solver.solve(model, t_eval)
        np.testing.assert_array_equal(solution.t, t_eval)
        np.testing.assert_allclose(solution["var1"].data[0], np.exp(0.1 * solution.t))
        np.testing.assert_allclose(
            solution["var2"].data[0], 2 * np.exp(0.1 * solution.t)
        )
