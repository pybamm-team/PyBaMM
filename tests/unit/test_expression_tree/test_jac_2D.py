...
class TestJacobian(unittest.TestCase):
    def test_linear(self):
        ...
        func = -v
        jacobian = np.zeros((4, 8))
        jacobian[:2, 2:4] = -np.eye(2)
        jacobian[2:4, 6:8] = -np.eye(2)
        dfunc_dy = func.jac(y).evaluate(y=y0)
        np.testing.assert_array_equal(jacobian, dfunc_dy.toarray())

        func = 3 * u + 4 * v
        jacobian = np.zeros((4, 8))
        jacobian[:2, :2] = 3 * np.eye(2)
        jacobian[2:4, 4:6] = 3 * np.eye(2)
        jacobian[:2, 2:4] = 4 * np.eye(2)
        jacobian[2:4, 6:8] = 4 * np.eye(2)
        dfunc_dy = func.jac(y).evaluate(y=y0)
        np.testing.assert_array_equal(jacobian, dfunc_dy.toarray())

        func = 7 * u - v * 9
        jacobian = np.zeros((4, 8))
        jacobian[:2, :2] = 7 * np.eye(2)
        jacobian[2:4, 4:6] = 7 * np.eye(2)
        jacobian[:2, 2:4] = -9 * np.eye(2)
        jacobian[2:4, 6:8] = -9 * np.eye(2)
        dfunc_dy = func.jac(y).evaluate(y=y0)
        np.testing.assert_array_equal(jacobian, dfunc_dy.toarray())
