...
class BasicDFN2D(BaseModel):
    ...
    def __init__(self, name="Doyle-Fuller-Newman model"):
        ...
        V_x = pybamm.FunctionParameter(
            "Electrolyte velocity X-component [m.s-1]",
            inputs={"x [m]": x, "z [m]": z},
        )
        V_z = pybamm.FunctionParameter(
            "Electrolyte velocity Z-component [m.s-1]",
            inputs={"x [m]": x, "z [m]": z},
        )
        ...
        N_e = pybamm.VectorField(
            ...
            pybamm.VectorField(V_x, V_z) * pybamm.UpwindDownwind2D(c_e, None, None)
        )
        self.rhs[c_e] = (1 / eps) * (
        ...
        self.variables = {
            ...
            "Electrolyte velocity X-component [m.s-1]": V_x,
            "Electrolyte velocity Z-component [m.s-1]": V_z,
...
# def test_basic_dfn_2d():
#    model = BasicDFN2D()
#    parameter_values = pybamm.ParameterValues("Chen2020")
#    parameter_values.update(
#    {
#        "Electrolyte velocity X-component [m.s-1]": 0,
#        "Electrolyte velocity Z-component [m.s-1]": 0,
#    },
#    check_already_exists=False,
# )
#    sim = pybamm.Simulation(model, parameter_values=parameter_values)
#    sol = sim.solve([0,1])
#    assert (sol.t >= 0.0).all()
