    ...
    def test_output_type(self):
        for function in self.function_list:
            if self.always_array is True:
                assert isinstance(function(0), numpy.typing.NDArray)
            else:
                assert isinstance(function(0), numbers.Number)
            assert isinstance(function(np.zeros(3)), numpy.typing.NDArray)
            assert isinstance(function(np.zeros([3, 3])), numpy.typing.NDArray)
