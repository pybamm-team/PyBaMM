...
class TestOldHomogeneousReaction(unittest.TestCase):
    ...
    def test_simplify_constant_current(self):
        ...
        # Simplifiy, since current is constant this should give an array when evaluated
        j_simp = j.simplify()
        self.assertIsInstance(j_simp.evaluate(t=0), np.ndarray)
        # test values
        l_n = parameter_values.process_symbol(param.l_n)
        l_p = parameter_values.process_symbol(param.l_p)
...
class TestOldButlerVolmer(unittest.TestCase):
    def setUp(self):
    ...
    def test_diff_c_e_lead_acid(self):
        ...
        model = pybamm.old_interface_lead_acid.OldMainReaction(param)
        parameter_values = model.default_parameter_values
...
class TestOldExchangeCurrentDensity(unittest.TestCase):
    def setUp(self):
    ...
    def test_creation_lithium_ion(self):
        param = pybamm.standard_parameters_lithium_ion
        model = pybamm.old_interface.OldLithiumIonReaction(param)
    ...
    def test_failure(self):
        model = pybamm.old_interface.OldLithiumIonReaction(None)
        with self.assertRaises(pybamm.DomainError):
