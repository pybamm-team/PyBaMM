...
class TestBasicSPMWith3DThermal:
    def test_spm_3d_vs_lumped(self):
        lumped_model = pybamm.lithium_ion.SPM(options={"thermal": "lumped"})

        three_d_model = pybamm.lithium_ion.BasicSPM_with_3DThermal(options={"cell geometry": "box", "dimensionality": 3})
        ...
        three_d_sim = pybamm.Simulation(three_d_model, parameter_values=params, var_pts=var_pts)

        lumped_sol = lumped_sim.solve([0, 3600])
        three_d_sol = three_d_sim.solve([0, 3600])

        np.testing.assert_allclose(
            lumped_sol.t[-1], three_d_sol.t[-1], rtol=0.01
        )
        
        lumped_temp_final = lumped_sol["Volume-averaged cell temperature [K]"].entries[-1]
        three_d_temp_final = three_d_sol["Volume-averaged cell temperature [K]"].entries[-1]
        np.testing.assert_allclose(
            lumped_temp_final, three_d_temp_final, rtol=0.01
        )

        lumped_volt_final = lumped_sol["Voltage [V]"].entries[-1]
        three_d_volt_final = three_d_sol["Voltage [V]"].entries[-1]
        np.testing.assert_allclose(
            lumped_volt_final, three_d_volt_final, rtol=0.01
        )
