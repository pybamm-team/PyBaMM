...
- Included a positive electrode degradation model for the SPM and DFN (P2D) that is enabled with the `"positive electrode degradation"` option. It is based of the shrinking-core model of phase transition and oxygen evolution of Ghosh et al. (2021) and the loss of active material as well as lithium inventory analysis of Zhuo et al. (2023). Includes the `Zhuo2023` parameter set for an NMC811/SiC 18650 cell and the `BoundaryCellValue` and `BoundaryCellLength` operators. ([#XXXX](https://github.com/pybamm-team/PyBaMM/pull/XXXX))
