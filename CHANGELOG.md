...
- Fixed the `pybammsolvers` Windows build failing to produce wheels: vcpkg's transitive reference-LAPACK dependency needed a Fortran toolchain whose pinned MSYS2 download has been pruned upstream (a 404). The sundials registry port's `klu` feature now depends only on `suitesparse-klu` (no LAPACK/Fortran, and no patched KLU discovery: SUNDIALS finds KLU via CMake config mode), and the Windows build uses vcpkg 2026.06.24 on a `windows-2025` runner. ([#5650](https://github.com/pybamm-team/PyBaMM/pull/5650))
