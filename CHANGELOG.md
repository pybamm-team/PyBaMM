...
- `BasicDFN2D` now uses the local active material volume fraction for the particle surface area, matching the solid lithium and electrode conductivity. It used the electrode-averaged fraction, so a spatially varying loading either failed to discretise (grading in x) or failed in the solver (grading in z). ([#XXXX](https://github.com/pybamm-team/PyBaMM/pull/XXXX))
