...
- Added `pybamm.Brent(residual, unknown, bounds)`, which solves `residual == 0` for a symbol within `bounds` by Brent's method. Every argument is an expression, so the bounds and anything inside `residual` may be an `InputParameter`. It converts to a CasADi `rootfinder` backed by a new native `brent` plugin in `pybammsolvers`, so the solve runs inside the CasADi graph with no Python interop, survives code generation and serialisation, and takes its derivatives from the implicit function theorem. Brent needs only a sign change over the bounds, so it converges where a Newton iteration stalls, and the answer cannot leave them. ([#5702](https://github.com/pybamm-team/PyBaMM/pull/5702))
