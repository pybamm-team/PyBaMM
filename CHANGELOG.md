...
- Requesting `"lithium plating": "partially reversible"` together with `"SEI": "none"` now raises a `pybamm.OptionError` at model construction instead of a bare `ZeroDivisionError` during parameter processing. The partially reversible plating model's dead lithium decay rate is inversely proportional to the SEI thickness, which is identically zero when the SEI submodel is disabled; this affected all four shipped parameter sets that use the SEI-limited decay function (`OKane2022`, `Ecker2015` and their half-cell variants). The error message points to the supported workaround of `"SEI": "constant"` with `"SEI film resistance": "none"`. ([#5790](https://github.com/pybamm-team/PyBaMM/pull/5790))
