...
- The defaults derived from `"SEI"`, `"SEI on cracks"` and `"lithium plating"` are now set per electrode. `"SEI": ("none", "none")` keeps `"SEI film resistance"` at `"none"` instead of switching it to `"distributed"`; `"SEI on cracks": ("false", "true")` or `("true", "true")` defaults `"particle mechanics"` to `"swelling and cracking"` on each electrode with cracks; and `"lithium plating": ("none", "partially reversible")` or `("partially reversible", "partially reversible")`, including inside a per-phase tuple, defaults `"SEI"` to `"constant"` on each electrode that plates. Previously only the scalar and negative-electrode tuple forms were recognised, so the other forms silently changed the model or raised an `OptionError` unless the dependent option was given explicitly. ([#XXXX](https://github.com/pybamm-team/PyBaMM/pull/XXXX))
