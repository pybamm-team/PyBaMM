...
- A differentiated `Interpolant`, such as the OCP derivative the heat of mixing takes, now works in the solvers for every 1D interpolator. A `"cubic"` one crashed CasADi when the solver formed its Jacobian, because the derivative spline's coefficients were passed with scipy's trailing padding, and a `"linear"` one (the default) could not be differentiated at all; its derivative is now the slope of the interval containing the point. So `"heat of mixing": "true"` works with a tabulated OCP or entropic change, as in `Ai2020`. ([#5798](https://github.com/pybamm-team/PyBaMM/issues/5798), [#5799](https://github.com/pybamm-team/PyBaMM/issues/5799), [#5804](https://github.com/pybamm-team/PyBaMM/pull/5804))
