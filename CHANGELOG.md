...
- `ProcessedVariable.as_computed()`, which adding a solution to one solved with `output_variables` relies on, lays each time point out as `IDAKLUSolver` returns it; 2D and 3D variables came back with their spatial axes reordered. ([#5785](https://github.com/pybamm-team/PyBaMM/pull/5785))
