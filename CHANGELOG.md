...
- Defaults derived from `"SEI"`, `"SEI on cracks"` and `"lithium plating"` are now resolved per electrode, including per-phase tuples. `"SEI": ("none", "none")` no longer turns on `"SEI film resistance"`, and positive-only or both-electrode SEI on cracks and partially reversible plating now get matching `"particle mechanics"` and `"SEI"` defaults instead of failing the option checks unless the dependent option was given explicitly. ([#XXXX](https://github.com/pybamm-team/PyBaMM/pull/XXXX))
