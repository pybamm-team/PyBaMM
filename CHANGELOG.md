...
- Building a battery model no longer changes the options dict passed to it. `SPM`, `MPM`, `NewmanTobias` and the lead-acid models wrote their defaults into it, and `BatteryModelOptions` rewrote `"SEI"`, `"SEI on cracks"` and `"lithium plating"` as tuples, so reusing one dict for several models changed the later ones: a `DFN` built after an `SPM` ran with x-averaged side reactions, and after an `MPM` with a particle-size distribution. `MPM` also no longer registers the Marquis2019 citation, which a class-to-string comparison made unconditional. ([#5800](https://github.com/pybamm-team/PyBaMM/issues/5800), [#5801](https://github.com/pybamm-team/PyBaMM/issues/5801), [#5806](https://github.com/pybamm-team/PyBaMM/pull/5806))
