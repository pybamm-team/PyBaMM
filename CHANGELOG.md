...
- IDAKLU diagnostics are emitted as the solve progresses whenever it runs on the calling thread — the default single-solver case, and any group with fewer input sets than solvers — so a long solve reports progress instead of going quiet until it returns. Only genuinely concurrent solves are held back to the end of the OpenMP region. ([#5717](https://github.com/pybamm-team/PyBaMM/pull/5717))
