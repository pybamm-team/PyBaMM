...
- Added a `newton_block_mode` option ("coupled" or "decoupled"; default "coupled") to `IDAKLUSolver` and `NonlinearSolver`. Under "decoupled" the algebraic Jacobian is split into its independent subsystems at setup and each carries its own Newton linesearch damping factor, so an ill-scaled equation no longer damps the whole state vector. ([#TBD](https://github.com/pybamm-team/PyBaMM/pull/TBD))
