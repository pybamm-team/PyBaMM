...
- The Newton solver for algebraic initial conditions now splits the algebraic Jacobian into its independent subsystems at setup and damps each with its own linesearch factor, so an ill-scaled equation no longer damps the whole state vector. Set the new `newton_block_mode` option ("decoupled" by default) to "coupled" to skip the analysis and restore the previous behaviour exactly. ([#TBD](https://github.com/pybamm-team/PyBaMM/pull/TBD))
