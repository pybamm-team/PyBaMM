...
- `ElectrodeSOHComposite` now solves each state as a bracketed rootfind on the shared electrode potential rather than as one nine-variable algebraic system handed to a Newton solver. Open-circuit potentials are monotone, so every solve is bracketed and converges from any starting point: ten composite-electrode states that previously failed now solve, and a 3480-call sweep over parameter perturbations, hysteresis branches and both initialisation methods returns no failures. Reading the results no longer goes through per-variable post-processing, which takes a repeated call from 850 ms to 3 ms. ([#TBD](https://github.com/pybamm-team/PyBaMM/pull/TBD))
- Added `pybamm.Brent`, a bracketed scalar rootfind backed by a native CasADi plugin. ([#TBD](https://github.com/pybamm-team/PyBaMM/pull/TBD))
- Converting a `Brent` to CasADi now shares its conversion cache with the expression around it instead of discarding everything converted inside the oracle, so a subexpression reachable from several rootfinds is converted once. Building the composite electrode SOH evaluator went from 308 rootfinder instantiations to 17, one per distinct `Brent`, and its cold build from 825 ms to 138 ms with results unchanged to the bit. ([#TBD](https://github.com/pybamm-team/PyBaMM/pull/TBD))
