...
- The per-variable sensitivities of an `output_variables` solve are keyed by the sensitivity parameters only. Every input used to take a key in turn, so an input no sensitivity was requested for, such as an experiment's step index, gained a spurious key, or took a parameter's column and shifted the labels after it. Each variable is also differentiated with respect to the sensitivity parameters alone: IDAKLU read the derivative with respect to every input as if it held only those, which gave wrong values when another input was stacked first and wrote past the end of a buffer otherwise. ([#5786](https://github.com/pybamm-team/PyBaMM/pull/5786))
