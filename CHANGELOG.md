...
- `convert_symbol_from_json` is strict again: raw strings, lists, and dicts without a `$type`/`type` tag raise `SerialisationError` (restoring pre-kernel validation) instead of being returned unchanged. Decoding a node missing a key its codec requires (e.g. minimal legacy JSON lacking `name`/`domains`) now raises a descriptive `SerialisationError` instead of a bare `KeyError`, and `SerialisationError` is importable as `pybamm.SerialisationError`. ([#5566](https://github.com/pybamm-team/PyBaMM/pull/5566))
