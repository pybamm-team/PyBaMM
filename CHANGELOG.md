...
- Fixed `pybamm.lithium_ion.DFN` (and adjacent reduced-order models) raising `NotImplementedError: method self.evaluate() not implemented for symbol r_n of type SpatialVariable` whenever `Initial concentration in <Domain> electrode [mol.m-3]` was set to a Python function of `(r, x)`. The user's callable evaluates to a symbolic expression in the `r` / `x` spatial variables, which then propagates through `c_init_av = xyz_average(r_average(c_init))` into the `Variable.reference` (`= -U_init`) of every electrolyte / electrode potential `Variable`. The discretisation substituted that `reference` (and `Variable.scale`) without itself discretising them first, so the un-folded `r_n` / `x_n` leaves leaked into the discretised rhs and initial-condition vectors and crashed `test_shape` / `check_initial_conditions`. `Discretisation._process_symbol` (for `Variable`, `VariableDot`, and `ConcatenationVariable`) and `Discretisation.process_equation` now discretise the symbol's `reference` and `scale` before combining them with the state-vector slice / initial-condition equation. The change is a no-op for the common scalar reference / scale (cached in `_discretised_symbols`) and unblocks callable initial concentrations on both `Simulation` and `Simulation`+`Experiment` build paths. ([#4930](https://github.com/pybamm-team/PyBaMM/issues/4930))
