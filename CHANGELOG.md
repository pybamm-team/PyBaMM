...
- A `pybammsolvers` solution saved with only output variables has `y_init`, its states at the first time point after consistent initialization, alongside `y_term`, and `yS_init` and `yS_term` with their sensitivities. ([#5794](https://github.com/pybamm-team/PyBaMM/pull/5794))
...
- `ProcessedVariableComputed` unrolls 3D variables time-major, the layout `IDAKLUSolver` returns, and reads a 2+1D model's current-collector nodes z-fastest, so a 3D variable requested through `output_variables` (such as `"Negative particle concentration distribution [mol.m-3]"`, or `"Electrolyte concentration [mol.m-3]"` in a 2+1D model) is no longer scrambled. ([#5785](https://github.com/pybamm-team/PyBaMM/pull/5785))
- `ProcessedVariable.as_computed()`, which adding a solution to one solved with `output_variables` relies on, lays each time point out as `IDAKLUSolver` returns it; 2D and 3D variables came back with their spatial axes reordered. It also converts a 0D time integral (`ExplicitTimeIntegral` or `DiscreteTimeSum`) to its single summed value, as `output_variables` stores it, instead of raising, and raises `NotImplementedError` for time integrals of spatially varying variables. ([#5785](https://github.com/pybamm-team/PyBaMM/pull/5785))
- Adding a solution solved with `output_variables` to another solution keeps the joined variables readable. When the second solution starts at the time the first ends, the joined variable drops the repeated time point as the solution does; it kept it, so reading the variable raised `ValueError: fp and xp are not of the same length`. A 0D `ExplicitTimeIntegral` joins into its integral over both solutions, counting the initial condition once; it became a variable with one value per solution that could not be read. A `DiscreteTimeSum`, or an expression of a time integral, has no value over joined solutions: the join still succeeds, including across the steps of an experiment, and reading the joined variable raises `NotImplementedError`. ([#5785](https://github.com/pybamm-team/PyBaMM/pull/5785))
...
- `first_state`, `last_state` and `step` of an `output_variables` solve with sensitivities use the states' sensitivities instead of the outputs', which gave wrong values or raised. ([#5794](https://github.com/pybamm-team/PyBaMM/pull/5794))
...
- `pybamm` now requires `pybammsolvers>=0.10.0`, because `IDAKLUSolver` uses IDAKLU binding features the 0.9 series does not have. Upgrade `pybammsolvers` together with `pybamm`. ([#5782](https://github.com/pybamm-team/PyBaMM/pull/5782), [#5783](https://github.com/pybamm-team/PyBaMM/pull/5783), [#5794](https://github.com/pybamm-team/PyBaMM/pull/5794))
...
- Retry `CasadiAlgebraicSolver` when it terminates early due to `step_tol`. ([#5457](https://github.com/pybamm-team/PyBaMM/pull/5457))
