...
- Added `pybamm.Brent(f, unknown, y_target, lo, hi)`, which solves `f == y_target` for a symbol in `[lo, hi]` by Brent's method. Every argument is an expression, so the target, the bracket and anything inside `f` may be an `InputParameter`. It converts to a CasADi `rootfinder` backed by a new native `brent` plugin in `pybammsolvers`, so the solve runs inside the CasADi graph with no Python interop, survives code generation and serialisation, and takes its derivatives from the implicit function theorem. Brent needs only a sign change over the bracket, so it converges where a Newton iteration stalls, and the answer cannot leave `[lo, hi]`. ([#5702](https://github.com/pybamm-team/PyBaMM/pull/5702))
