...
for i in range(TOTAL_CYCLES):
    ...
    d_cap = step_discharge["Discharge capacity [A.h]"].entries[-1] - step_discharge["Discharge capacity [A.h]"].entries[0]
    
    # Step 3: Charge CC
    step_charge_cc = sol.steps[2]
    c_cap_cc = step_charge_cc["Discharge capacity [A.h]"].entries[-1] - step_charge_cc["Discharge capacity [A.h]"].entries[0]
    
    # Step 4: Charge CV
    step_charge_cv = sol.steps[3]
    c_cap_cv = step_charge_cv["Discharge capacity [A.h]"].entries[-1] - step_charge_cv["Discharge capacity [A.h]"].entries[0]
...
fig_plotly.add_trace(go.Scatter(x=cycle_numbers, y=discharge_capacities, mode='lines+markers', name='Discharge Capacity'))
fig_plotly.add_trace(go.Scatter(x=cycle_numbers, y=cc_charge_capacities, mode='lines+markers', name='CC Charge Capacity'))
fig_plotly.add_trace(go.Scatter(x=cycle_numbers, y=cv_charge_capacities, mode='lines+markers', name='CV Charge Capacity'))
...
for i in range(len(discharge_capacities)):
    if i < 5 or i >= len(discharge_capacities) - 5:
        print(f"{cycle_numbers[i]:<6} | {discharge_capacities[i]:<16.4f} | {cc_charge_capacities[i]:<16.4f} | {cv_charge_capacities[i]:<16.4f}")
    elif i == 5:
        print("...")
