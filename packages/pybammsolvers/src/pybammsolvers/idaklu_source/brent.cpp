...
// the iteration, compiled here and stringified by CMake into brent_impl_str
...
void Brent::codegen_declarations(CodeGenerator& g) const {
  ...
  // one definition per file, not per node; add_shorthand stays off so the name is not
  // CASADI_PREFIX-renamed, which would defeat the guard
  ...
  // the same cache as BrentMemory, per instance and per thread
