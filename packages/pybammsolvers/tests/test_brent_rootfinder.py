...
class TestBrentPlugin:
    ...
    @pytest.mark.parametrize(
        "expr_fn",
        [
            lambda x, p: casadi.exp(x) + p * x - 2.0,
            # non-monotone: several turning points inside the bracket, which is where
            # a Newton iteration stalls
            lambda x, p: casadi.sin(12 * x) * 0.08 + (1.6 - x) ** 3 - p,
        ],
    )
