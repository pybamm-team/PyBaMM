...
def graphite_LGM50_diffusivity_Chen2020(sto, T):
    ...
    Arrhenius temperature dependence). D_ref and activation energy for the composite
    graphite phase are taken from :footcite:t:`Bonkile2024` (D_ref = 3.3e-14 m2.s-1,
    E_D_s = 3.03e4 J.mol-1).
...
def silicon_LGM50_diffusivity_Bonkile2024(sto, T):
    """
    Silicon diffusivity as a function of stoichiometry (taken as constant, with
    Arrhenius temperature dependence). D_ref and activation energy for the composite
    silicon phase are taken from :footcite:t:`Bonkile2024` (D_ref = 3.0e-16 m2.s-1,
    E_D_s = 4.82e4 J.mol-1, after Jiang et al. 2022 / Johari et al. 2011).
...
def silicon_cracking_rate_Ai2020(T_dim):
    ...
    cracking rate as graphite (3.9e-20) per :footcite:t:`Bonkile2024`.
...
def nmc_LGM50_diffusivity_Chen2020(sto, T):
    """
     ...
     temperature dependence). The value is taken from [1].
...
def volume_change_Ai2020(sto):
    """
    Positive particle volume change as a function of stoichiometry [1, 2].
...
def cracking_rate_Ai2020(T_dim):
    """
    Positive particle cracking rate as a function of temperature [1, 2].
...
def get_parameter_values():
    ...
    return {
        ...
        "Primary: Dead lithium decay rate [s-1]": SEI_limited_dead_lithium_OKane2022,
        ...
        "Primary: Initial SEI on cracks thickness [m]": 5e-13,  # practically zero
        ...
        "Primary: SEI growth activation energy [J.mol-1]": 38000.0,
        ...
        "Secondary: Initial SEI on cracks thickness [m]": 5e-13,  # practically zero
        ...
        "Primary: Initial concentration in negative electrode [mol.m-3]": 23000.0,
        "Primary: Negative particle diffusivity [m2.s-1]"
        ...
        "Secondary: Negative particle diffusivity [m2.s-1]"
        ...
        # negative electrode mechanics / cracking / LAM (primary: graphite)
        "Primary: Negative electrode Poisson's ratio": 0.3,
        "Primary: Negative electrode Young's modulus [Pa]": 15000000000.0,
        "Primary: Negative electrode reference concentration for free of deformation "
        "[mol.m-3]": 0.0,
        ...
        "Primary: Negative electrode number of cracks per unit area [m-2]": 3.18e15,
        ...
        # negative electrode mechanics / cracking / LAM (secondary: silicon)
        ...
        "Secondary: Negative electrode critical stress [Pa]": 720000000.0,
        # positive electrode
        ...
        # positive electrode mechanics / cracking / LAM
