    ...
    * **3D**: tetrahedra (4 vertices) or hexahedra (8 vertices)
    ...
    def _compute_cell_geometry(self):
        ...
        if self.element_type == "triangle":
            ...
            # 5-tet split with consistent positive orientation: mixed signs
            # within a cell expose invalid vertex ordering.
    ...
    def _build_face_connectivity(self):
        ...
        # numpy 2.0.0 returns a 2D inverse for axis != None
    ...
    def optimize_ordering(self):
        ...
        for data_dict in self.interface_data.values():
            ...
            if other is None:
                continue
            for other_data in other.interface_data.values():
                if other_data.get("other_mesh") is self:
                    other_data["right_cells"] = inv_perm[other_data["right_cells"]]
    ...
    def boundary_loops(self):
        ...
        Path = pybamm.import_optional_dependency("matplotlib.path", "Path")
...
class UnstructuredMeshGenerator(MeshGenerator):
    ...
    * **3D**: rectangular prism meshed by splitting each hex into 5
    ...
    element_type : str, optional
        ``"quad"`` for quadrilateral cells (2D, TPFA-orthogonal),
        ``"triangle"`` for triangular cells (2D default),
        ``"hexahedron"`` for hexahedral cells (3D default, TPFA-orthogonal),
        ``"tetrahedron"`` for tetrahedral cells (3D).
    """
