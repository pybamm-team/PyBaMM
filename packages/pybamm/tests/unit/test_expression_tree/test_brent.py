...
class TestBrent:
    def test_solves_a_scalar_equation(self):
        ...
        node = pybamm._Brent(pybamm.exp(x) + x - 2.0, x, (-5.0, 5.0))
    ...
    def test_target_may_be_an_input_parameter(self):
        x = pybamm._BrentUnknown("x")
        node = pybamm._Brent(x * x - pybamm.InputParameter("target"), x, (0.0, 10.0))
    ...
    def test_the_expression_may_contain_input_parameters(self):
        ...
        x = pybamm._BrentUnknown("x")
        node = pybamm._Brent(pybamm.InputParameter("a") * x * x - 9.0, x, (0.0, 10.0))
    ...
    def test_solves_over_the_state_vector(self):
        ...
        x = pybamm._BrentUnknown("x")
        node = pybamm._Brent(x * state - 6.0, x, (0.0, 10.0))
        y = casadi.MX.sym("y", 1)
    ...
    def test_composes_into_a_larger_expression(self):
        x = pybamm._BrentUnknown("x")
        node = pybamm._Brent(x * x - 9.0, x, (0.0, 10.0))
    ...
    def test_nests(self):
        ...
        inner_x = pybamm._BrentUnknown("inner")
        inner = pybamm._Brent(inner_x * inner_x - 16.0, inner_x, (0.0, 10.0))
        outer_x = pybamm._BrentUnknown("outer")
    ...
    def test_evaluating_does_not_re_enter_python(self):
        ...
        x = pybamm._BrentUnknown("x")
        node = pybamm._Brent(pybamm.exp(x) + x * state - 2.0, x, (-5.0, 5.0))
    ...
    def test_errors(self):
        x = pybamm._BrentUnknown("x")
        with pytest.raises(TypeError, match=r"unknown must be a _BrentUnknown"):
            pybamm._Brent(x * x - 9.0, 1.0, (0, 1))
        with pytest.raises(TypeError, match=r"residual must be a pybamm\.Symbol"):
            pybamm._Brent(1.0, x, (0, 1))
        with pytest.raises(pybamm.ModelError, match="does not appear in"):
            pybamm._Brent(pybamm.Scalar(2) * pybamm.t - 9.0, x, (0, 1))
        with pytest.raises(pybamm.ModelError, match="bounds must be a"):
            pybamm._Brent(x * x - 9.0, x, (0, 1, 2))
    ...
    def test_round_trips_through_json(self):
        x = pybamm._BrentUnknown("x")
        node = pybamm._Brent(x * x - 9.0, x, (0.0, 10.0), abstol=1e-12, max_iter=42)
        ...
        assert (
            len(
                {s.name for s in rebuilt.pre_order() if type(s) is pybamm._BrentUnknown}
            )
            == 1
        )
        assert float(casadi.evalf(rebuilt.to_casadi(inputs={}))) == pytest.approx(3.0)

    def test_an_unknown_outside_its_brent_is_an_error(self):
        # the Brent binds it in `inputs` only while converting its own residual
        with pytest.raises(
            pybamm.ModelError, match="only has a value inside the Brent"
        ):
            pybamm._BrentUnknown("x").to_casadi(inputs={})

    def test_the_tolerances_are_part_of_the_identity(self):
        # a Brent converted inside another's oracle is shared with the enclosing
        # conversion, so two differing only in tolerance must not be served each other
        unknown = pybamm._BrentUnknown("s")
        target = pybamm.InputParameter("p")
        residual = unknown * unknown - target
        coarse = pybamm._Brent(residual, unknown, (0.01, 10), abstol=1e-1, max_iter=100)
        fine = pybamm._Brent(residual, unknown, (0.01, 10), abstol=1e-14, max_iter=100)
        assert coarse.id != fine.id

        outer_unknown = pybamm._BrentUnknown("outer")
        outer = pybamm._Brent(outer_unknown - coarse, outer_unknown, (0.01, 50))
