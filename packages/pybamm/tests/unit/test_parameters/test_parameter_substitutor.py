...
class TestProcessSymbol:
    ...
    def test_process_function_parameter_keeps_direct_interpolant_children(self):
        ...
        # a directly supplied interpolant is used as given, not rebound to the
        # function parameter's children; it is broadcast onto their domain instead
        assert processed_interpolant.children[0] is original_time
        assert interpolant.children[0] is original_time
        assert original_time.domains == original_domains
        assert result.domains == time.domains
        np.testing.assert_allclose(
            processed_interpolant.evaluate(t=0.5), 298.65, rtol=1e-12
        )
