...
def load_experimental_data():
    ...
    try:
        ...
        print(f"  Voltage range: {voltage_data.min():.4f} to {voltage_data.max():.4f} V")
...
def run_single_msmr_simulation(time_data, current_data, voltage_data):
    ...
    [
        ...
        print(f"Simulation voltage range: {voltage_sim.min():.4f} to {voltage_sim.max():.4f} V")
        print(f"Simulation capacity range: {capacity_sim.min():.6f} to {capacity_sim.max():.6f} Ah")
...
def calculate_mse(exp_data, sim_data):
    ...
    if len(exp_data) != len(sim_data):
        ...
        sim_interp = interp1d(np.linspace(0, 1, len(sim_data)), sim_data, 
...
def plot_comparison(time_exp, voltage_exp, capacity_exp, 
    ...
    fig.suptitle('MSMR Simulation vs Experimental Data Comparison', fontsize=16, fontweight='bold')
    
    # Plot 1: Voltage vs Time
    ax1.plot(time_exp, voltage_exp, 'b-', label='Experimental', linewidth=1.5, alpha=0.8)
    ...
    ax1.text(0.02, 0.98, f'MSE: {voltage_mse:.6f}', transform=ax1.transAxes, 
             verticalalignment='top', bbox=dict(boxstyle='round', facecolor='lightblue', alpha=0.8))
    
    # Plot 2: Capacity vs Time
    ax2.plot(time_exp, capacity_exp, 'g-', label='Experimental (Normalized)', linewidth=1.5, alpha=0.8)
    ax2.plot(time_sim, capacity_sim_norm, 'm--', label='MSMR Simulation (Normalized)', linewidth=2)
    ax2.set_xlabel('Time [s]')
    ax2.set_ylabel('Normalized Capacity')
    ax2.set_title('Normalized Capacity vs Time')
    ax2.legend()
    ax2.grid(True, alpha=0.3)
    ax2.text(0.02, 0.98, f'MSE: {capacity_mse:.6f}', transform=ax2.transAxes, 
             verticalalignment='top', bbox=dict(boxstyle='round', facecolor='lightgreen', alpha=0.8))
    
    # Plot 3: Voltage vs Capacity (Normalized)
    ax3.plot(capacity_exp, voltage_exp, 'b-', label='Experimental', linewidth=1.5, alpha=0.8)
    ax3.plot(capacity_sim_norm, voltage_sim, 'r--', label='MSMR Simulation', linewidth=2)
    ax3.set_xlabel('Normalized Capacity')
    ax3.set_ylabel('Voltage [V]')
    ax3.set_title('Voltage vs Normalized Capacity')
    ax3.legend()
    ax3.grid(True, alpha=0.3)
    ax3.text(0.02, 0.98, f'Total MSE: {total_mse:.6f}', transform=ax3.transAxes, 
             verticalalignment='top', bbox=dict(boxstyle='round', facecolor='lightyellow', alpha=0.8))
    
    # Plot 4: Raw Capacity vs Time
    ax4.plot(time_sim, capacity_sim_raw, 'purple', label='MSMR Raw Capacity', linewidth=2)
    ax4.set_xlabel('Time [s]')
    ax4.set_ylabel('Capacity [Ah]')
    ax4.set_title('Raw Simulation Capacity vs Time')
    ax4.legend()
    ax4.grid(True, alpha=0.3)
    ax4.text(0.02, 0.98, f'Max Capacity: {capacity_sim_raw.max():.4f} Ah', transform=ax4.transAxes, 
...
def main():
    ...
    time_sim, voltage_sim, capacity_sim_norm, capacity_sim_raw = run_single_msmr_simulation(
        time_exp, current_exp, voltage_exp)
    ...
    voltage_mse, capacity_mse = plot_comparison(
        time_exp, voltage_exp, capacity_exp,
        time_sim, voltage_sim, capacity_sim_norm, capacity_sim_raw
    )
    ...
    if total_mse < 0.01:
        print(f"  🎉 Excellent fit! MSE < 0.01")
    elif total_mse < 0.05:
        print(f"  ✅ Good fit! MSE < 0.05")
    elif total_mse < 0.1:
        print(f"  ⚠️  Fair fit. MSE < 0.1")
    else:
        print(f"  ❌ Poor fit. MSE >= 0.1")
