...
def load_experimental_data():
    ...
    try:
        ...
        print(
            f"Capacity range: {capacity_experimental.min():.6f} to {capacity_experimental.max():.6f}"
        )
        print(
            f"Voltage range: {voltage_experimental.min():.6f} to {voltage_experimental.max():.6f}"
        )
        ...
        voltage_experimental = (
            4.2
            - 1.2 * capacity_experimental
            + 0.3 * np.sin(2 * np.pi * capacity_experimental)
        )
...
def load_all_data():
    ...
    try:
        ...
        voltage_data = (
            4.2
            - 1.2 * (time_data / time_data[-1])
            + 0.3 * np.sin(2 * np.pi * time_data / time_data[-1])
        )
...
def run_msmr_simulation(params_dict):
    ...
    try:
        ...
        if (
            np.any(np.isnan(voltage_sim))
            or np.any(np.isnan(capacity_sim))
            or np.any(np.isnan(time_sim))
        ):
            raise ValueError("NaN values in simulation results")

        if (
            np.any(np.isinf(voltage_sim))
            or np.any(np.isinf(capacity_sim))
            or np.any(np.isinf(time_sim))
        ):
...
def debug_cost_calculation(params_dict):
    ...
    print(
        f"Experimental capacity range: {capacity_exp.min():.6f} to {capacity_exp.max():.6f}"
    )
    print(
        f"Experimental voltage range: {voltage_exp.min():.6f} to {voltage_exp.max():.6f}"
    )
    ...
    print(
        f"Simulation capacity range: {capacity_sim.min():.6f} to {capacity_sim.max():.6f}"
    )
    print(
        f"Simulation voltage range: {voltage_sim.min():.6f} to {voltage_sim.max():.6f}"
    )
    ...
    if len(capacity_sim) > 1:
        ...
        if exp_min_in_sim and exp_max_in_sim:
            ...
            print(
                f"Interpolated voltage range: {voltage_sim_interp.min():.6f} to {voltage_sim_interp.max():.6f}"
            )
            ...
            for i in range(min(10, len(capacity_exp))):
                print(
                    f"  Point {i}: Exp={voltage_exp[i]:.4f}V, Sim={voltage_sim_interp[i]:.4f}V, Diff={abs(voltage_exp[i] - voltage_sim_interp[i]):.4f}V"
                )
...
def sample_current_data(time_data, current_data, voltage_data, max_points=100):
    ...
    print(
        f"Current statistics: mean={current_mean:.6f}A, std={current_std:.6f}A, CV={current_cv:.3f}"
    )
...
def plot_sampled_data():
    ...
    if sampling_info["use_constant_current"]:
        ...
        ax1.plot(
            time_data[sample_indices],
            current_data[sample_indices],
            "ro",
            markersize=4,
        ...
        ax1.plot(
            sampling_info["time_sampled"],
            sampling_info["current_sampled"],
            "ro",
            label=f"Sampled Data ({sampling_info['sampled_points']} points)",
            markersize=4,
        )
    ...
    ax2.plot(
        time_data, voltage_data, "g-", label="Full Experimental Voltage", linewidth=1
    )
    ...
    stats_text += (
        f"Voltage Range: {voltage_data.min():.3f}V to {voltage_data.max():.3f}V"
    )
...
def optimize_msmr_parameters():
    ...
    optimizer = pybamm.BayesianOptimization(
        f=calculate_cost, pbounds=pbounds, random_state=42, verbose=2
    )
...
def plot_results(optimizer):
    ...
    for idx, param_name in enumerate(param_names):
        param_values = [
            optimizer.space.params[i][idx] for i in range(len(optimizer.space))
        ]
...
def plot_solution_distribution(optimizer, experimental_data, best_params, ax):
    ...
    for i in range(len(optimizer.space)):
        try:
            ...
            alpha = (
                0.1 if i != len(optimizer.space) - 1 else 0.8
            )  # Make best solution more visible
            color = (
                "red" if i == len(optimizer.space) - 1 else "blue"
            )  # Best solution in red
            linewidth = 2 if i == len(optimizer.space) - 1 else 0.5
            ax.plot(
                capacity_sim, voltage_sim, color=color, alpha=alpha, linewidth=linewidth
            )
            successful_count += 1
        except:
    ...
    ax.text(
        0.05,
        0.95,
        f"Successful simulations: {successful_count}/{total_sims}",
...
def plot_best_vs_worst(optimizer, experimental_data, ax):
    ...
    try:
        ...
        ax.plot(
            ...
            voltage_best,
            "g-",
            label=f"Best (MSE: {-costs[best_idx]:.4f})",
            linewidth=2,
        )
    except:
        pass
    ...
    try:
        ...
        ax.plot(
            ...
            voltage_worst,
            "r--",
            label=f"Worst (MSE: {-costs[worst_idx]:.4f})",
            linewidth=2,
        )
    except:
        pass
...
def plot_experiment_vs_model():
    ...
    mae, mse, voltage_sim_interp = calculate_mae_mse(
        voltage_data, voltage_sim, time_data, sim_time
    )
    ...
    ax1.plot(
        ...
        "go",
        markersize=4,
        label=f"Experimental Sample Points ({len(sample_indices)})",
    )
...
def optimize_c_rate():
    ...
    ax1.axvline(
        ...
        linestyle="--",
        linewidth=2,
        label=f"Best: C/{1 / best_c_rate:.1f}",
    )
    ...
    ax2.plot(
        ...
        voltage_sim_best,
        "r-",
        label=f"MSMR Model (C/{1 / best_c_rate:.1f})",
        linewidth=2,
    )
    ax2.set_xlabel("Time [s]")
    ax2.set_ylabel("Voltage [V]")
...
def multi_objective_optimization():
    ...
    c_rates = np.linspace(
        1 / 20, 1 / 5, 100
    )  # From C/20 to C/5, more points for better resolution
    results = []
    ...
    normalized_terrs = [
        (terr - terr_min) / (terr_max - terr_min) for terr in time_errors
    ]
    ...
    for i, result in enumerate(valid_results):
        ...
        for j, other in enumerate(valid_results):
            if i != j:
                if (
                    other["mae"] <= result["mae"]
                    and other["time_error"] <= result["time_error"]
                    and (
                        other["mae"] < result["mae"]
                        or other["time_error"] < result["time_error"]
                    )
                ):
                    dominated = True
                    break
    ...
    for i, solution in enumerate(pareto_front):
        print(
            f"  {i + 1}. C/{1 / solution['c_rate']:.1f}: MAE = {solution['mae']:.6f} V, |ΔT| = {solution['time_error']:.1f} s"
        )
    ...
    print("\n🏆 Best Solutions:")
    print(
        f"  Best MAE: C/{1 / best_mae_solution['c_rate']:.1f} (MAE = {best_mae_solution['mae']:.6f} V, |ΔT| = {best_mae_solution['time_error']:.1f} s)"
    )
    print(
        f"  Best |ΔT|: C/{1 / best_time_solution['c_rate']:.1f} (MAE = {best_time_solution['mae']:.6f} V, |ΔT| = {best_time_solution['time_error']:.1f} s)"
    )
    ...
    scatter = ax1.scatter(
        terrs_plot, maes_plot, c=c_rates_plot, cmap="viridis", s=50, alpha=0.7
    )
    ax1.scatter(
        [r["time_error"] for r in pareto_front],
    ...
    ax1.scatter(
        best_mae_solution["time_error"],
        best_mae_solution["mae"],
        c="green",
        s=200,
        marker="o",
        label="Best MAE",
    ...
    ax1.scatter(
        best_time_solution["time_error"],
        best_time_solution["mae"],
        c="blue",
        s=200,
        marker="s",
        label="Best |ΔT|",
    ...
    ax2.plot(
        [1 / r["c_rate"] for r in valid_results],
        [r["mae"] for r in valid_results],
        "bo-",
        linewidth=2,
    )
    ax2.axvline(
        x=1 / best_mae_solution["c_rate"],
        color="green",
        linestyle="--",
        linewidth=2,
        label=f"Best MAE: C/{1 / best_mae_solution['c_rate']:.1f}",
    )
    ...
    ax3.plot(
        [1 / r["c_rate"] for r in valid_results],
        [r["time_error"] for r in valid_results],
        "ro-",
        linewidth=2,
    )
    ax3.axvline(
        x=1 / best_time_solution["c_rate"],
        color="blue",
        linestyle="--",
        linewidth=2,
        label=f"Best |ΔT|: C/{1 / best_time_solution['c_rate']:.1f}",
    )
    ...
    ax4.plot(
        time_data, voltage_data, "k-", label="Experimental", linewidth=1, alpha=0.7
    )

    # Show best MAE solution
    ax4.plot(
        best_mae_solution["sim_time"],
        best_mae_solution["voltage_sim"],
        "g-",
        label=f"Best MAE (C/{1 / best_mae_solution['c_rate']:.1f})",
        linewidth=2,
    )
    ...
    ax4.plot(
        best_time_solution["sim_time"],
        best_time_solution["voltage_sim"],
        "b--",
        label=f"Best |ΔT| (C/{1 / best_time_solution['c_rate']:.1f})",
        linewidth=2,
    )
