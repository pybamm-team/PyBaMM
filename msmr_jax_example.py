...
def main():
    ...
    options = {
        "number of MSMR reactions": ("1", "1"),  # (negative electrode, positive electrode)
        "particle": "MSMR",
    ...
    try:
        ...
        print(f"   Final voltage: {solution['Voltage [V]'].data[-1]:.3f} V")
        print(f"   Voltage drop: {solution['Voltage [V]'].data[0] - solution['Voltage [V]'].data[-1]:.3f} V")
    ...
    try:
        ...
        try:
            ...
            initial_capacity = parameter_values.evaluate(pybamm.Parameter("Nominal cell capacity [A.h]"))
        ...
        # Save the plot
        plt.savefig('/Users/mohitmehta/git_downloads/PyBaMM/msmr_jax_results.png', 
        ...
        try:
            sim.plot()
            plt.savefig('/Users/mohitmehta/git_downloads/PyBaMM/msmr_jax_basic_plot.png')
