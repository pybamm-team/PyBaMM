...
def graphite_electrolyte_exchange_current_density_Ecker2015(c_e, c_s_surf, T):
    ...
    k_ref = Scalar(1.995 * 1e-10, "[mol.s.m-2]")

    # multiply by Faraday's constant to get correct units
    m_ref = constants.F * k_ref  # (A/m2)(m3/mol)**1.5 - includes ref concentrations
    E_r = Scalar(53400, "[J.mol-1]")

    arrhenius = exp(-E_r / (constants.R * T)) * exp(
        E_r / (constants.R * Scalar(296.15, "[K]"))
    )
