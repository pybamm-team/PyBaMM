...
def get_julia_function(
    ...
    while var_symbols:
        ...
        # Look for lists in the variable symbols. These correspond to concatenations, so
        ...
        if symbol_line[0] == "[" and symbol_line[-1] == "]":
            ...
            new_list = []
            #make sure we haven't split a tuple
            kill=False
            for i,thing in enumerate(symbol_line):
                if kill:
                    continue
                elif thing[0] == '(':
                    next_thing = symbol_line[i+1]
                    thing = thing+next_thing
                new_list.append(thing)
            symbol_line = new_list
            start = 0
