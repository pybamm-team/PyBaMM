...
class Simulation:
    ...
    def set_up_experiment(self, model, experiment):
        ...
        # Process capacity and timescale
        nominal_capacity = self._parameter_values["Nominal cell capacity [A.h]"]
        timescale = self._parameter_values.evaluate(model.timescale)
        experiment = experiment.process_parameters(nominal_capacity, timescale)
    ...
    def set_up_model_for_experiment(self, model):
        """
        Set up self.model to be able to run the experiment.
        A new model is created for each step.
