...
class Model:
    """The parent model class. This contains variables and methods common to all Models
        ...
        """
    #   will be useful to have current and BV stored somewhere for reference as needed for RHS
    ...
    def initial_conditions(self, param, mesh):
        for i in self.sub_models.keys():
            self.sub_models[i].initial_conditions(list(self.sub_models.keys())[i], param, mesh)
...
# Each specific model should just contain a list of the domains and the
# type of model to use on each domain
class SPM(Model):
...
class UserModel(Model):
    """A class so that a user can mix and match domains and submodels by only entering
    a dictionary of sub_models. All the functionality of Model is then retained."""
