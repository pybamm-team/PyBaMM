...
class BasicPSDModel(BaseModel):
    ...
    **Extends:** :class:`pybamm.lithium_ion.BaseModel`
    ...
    def __init__(self, name="Particle-Size Distribution Model"):
        ...
        sd_a_n = pybamm.Parameter(
            "negative area-weighted particle-size standard deviation"
        )
        sd_a_p = pybamm.Parameter(
            "positive area-weighted particle-size standard deviation"
        )
        ...
        def f_a_dist_n(R, R_av_a, sd_a):
            ...
            return pybamm.FunctionParameter(
                "negative area-weighted particle-size distribution", inputs,
            )
        ...
        def f_a_dist_p(R, R_av_a, sd_a):
            ...
            return pybamm.FunctionParameter(
                "positive area-weighted particle-size distribution", inputs,
            )
        ...
        # X-averaged particle concentrations: these now depend continuously on particle
        # size with secondary domains "negative/positive particle-size domain"
        ...
        phi_e = pybamm.Variable("Electrolyte potential")
        ...
        j0_n = param.j0_n(1, c_s_surf_n, T) / param.C_r_n  # set c_e = 1
        j_n = (
            2
            * j0_n
            * pybamm.sinh(param.ne_n / 2 * (phi_s_n - phi_e - param.U_n(c_s_surf_n, T)))
        )
        c_s_surf_p = pybamm.surf(c_s_p)
        j0_p = (
            param.gamma_p * param.j0_p(1, c_s_surf_p, T) / param.C_r_p
        )  # setting c_e = 1

        j_p = (
            2
            * j0_p
            * pybamm.sinh(param.ne_p / 2 * (phi_s_p - phi_e - param.U_p(c_s_surf_p, T)))
        )
        ...
        self.algebraic[phi_s_p] = (
            pybamm.Integral(f_a_dist_p(R_variable_p, 1, sd_a_p) * j_p, R_variable_p)
            + i_cell / param.l_p
        )

        self.initial_conditions[phi_e] = pybamm.Scalar(
            1
        )  # pybamm.PrimaryBroadcast(1, "negative particle-size domain")
        self.initial_conditions[phi_s_p] = pybamm.Scalar(
            1
        )  # pybamm.PrimaryBroadcast(1, "positive particle-size domain")
        ...
        c_s_n_size_av = pybamm.Integral(
            f_a_dist_n(R_variable_n, 1, sd_a_n) * c_s_n, R_variable_n
        )
        c_s_p_size_av = pybamm.Integral(
            f_a_dist_p(R_variable_p, 1, sd_a_p) * c_s_p, R_variable_p
        )
        c_s_surf_n_size_av = pybamm.Integral(
            f_a_dist_n(R_variable_n, 1, sd_a_n) * c_s_surf_n, R_variable_n
        )
        c_s_surf_p_size_av = pybamm.Integral(
            f_a_dist_p(R_variable_p, 1, sd_a_p) * c_s_surf_p, R_variable_p
        )
        ...
        self.variables.update(
            {
                # (Some of) New "Distribution" variables, those depending on R_variable_n, R_variable_p
                ...
                "Negative area-weighted particle-size distribution": f_a_dist_n(
                    R_variable_n, 1, sd_a_n
                ),
                "Positive area-weighted particle-size distribution": f_a_dist_p(
                    R_variable_p, 1, sd_a_p
                ),
    ...
    def default_parameter_values(self):
        ...
        # Area-weighted standard deviations (dimensionless)
        sd_a_n = 0.5
        sd_a_p = 0.3
