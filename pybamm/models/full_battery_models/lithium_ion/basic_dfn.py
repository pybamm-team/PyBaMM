...
class BasicDFN(BaseModel):
    ...
    **Extends:** :class:`pybamm.lithium_ion.BaseModel`
    ...
    def __init__(self, name="Doyle-Fuller-Newman model"):
        ...
        phi_s_p_ohm = pybamm.Variable(
            "Positive electrode potential [V]",
            domain="positive electrode",
        )
        V = pybamm.Variable("Terminal voltage [V]", scale=param.ocv_init)
        phi_s_p = V + phi_s_p_ohm
        ...
        # Current in the solid
        ######################
        sigma_eff_n = 1 + 0 * param.n.sigma(T) * eps_s_n**param.n.b_s
        i_s_n = -sigma_eff_n * pybamm.grad(phi_s_n)
        sigma_eff_p = 1 + 0 * param.p.sigma(T) * eps_s_p**param.p.b_s
        i_s_p = -sigma_eff_p * pybamm.grad(phi_s_p_ohm)
        # The `algebraic` dictionary contains differential equations, with the key being
        # the main scalar variable of interest in the equation
        self.algebraic[phi_s_n] = param.L_x**2 * (pybamm.div(i_s_n) + a_j_n)
        self.algebraic[phi_s_p_ohm] = param.L_x**2 * (pybamm.div(i_s_p) + a_j_p)
        self.boundary_conditions[phi_s_n] = {
            "left": (pybamm.Scalar(0), "Dirichlet"),
            "right": (pybamm.Scalar(0), "Neumann"),
        }
        self.boundary_conditions[phi_s_p_ohm] = {
            "left": (pybamm.Scalar(0), "Neumann"),
        ...
        # conditions
        self.initial_conditions[phi_s_n] = pybamm.Scalar(0)
        self.initial_conditions[phi_s_p_ohm] = pybamm.Scalar(0)

        x_p = pybamm.SpatialVariable("x_p", domain=["positive electrode"])
        self.algebraic[V] = pybamm.Integral(a_j_p, x_p) + i_cell
        # self.algebraic[V] = (
        #     sigma_eff_p * pybamm.boundary_gradient(phi_s_p_ohm, "right") + i_cell
        # )
        self.initial_conditions[V] = param.ocv_init
