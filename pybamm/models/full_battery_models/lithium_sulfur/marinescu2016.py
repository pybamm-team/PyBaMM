...
class MarinescuEtAl2016(BaseModel):
    ...
    def __init__(self, options=None, name="Marinescu et al. (2016) model"):
        ...
        dSpdt = (2 * ns * Ms * i_L / (ne * F)) - k_p * Sp * (S - S_star) / (v * rho_s)

        # Differential equation (8e) in [1]
        dSdt = k_p * Sp * (S - S_star) / (v * rho_s)
        ...
        self.variables.update(
            {
                ...
                "Sum of high and low plateau currents [A]": i_H + i_L,
                "Theoretical capacity [Ah]": cth,
                "Algebraic condition": i_H + i_L - I,
            }
        )
        ...
        """
        S8_initial = param.S8_initial
        S4_initial = param.S4_initial
        S2_initial = param.S2_initial
        S_initial = param.S_initial
        Sp_initial = param.Sp_initial
        V_initial = param.V_initial

        self.initial_conditions.update(
            {
                self.variables["S8 [g]"]: S8_initial,
                self.variables["S4 [g]"]: S4_initial,
                self.variables["S2 [g]"]: S2_initial,
                self.variables["S [g]"]: S_initial,
                self.variables["Precipitated Sulfur [g]"]: Sp_initial,
                self.variables["Terminal voltage [V]"]: V_initial,
            }
        )
