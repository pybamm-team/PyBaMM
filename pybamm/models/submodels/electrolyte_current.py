    ...
    *Extends:* :class:`ElectrolyteCurrentBaseModel`
    ...
    def set_leading_order_system(self, delta_phi, reactions, domain):
        ...
        if domain == ["negative electrode"]:
            ...
            # sum_j = sum(rxn["neg"]["aj"] for rxn in reactions.values())
            sum_j = reactions["main"]["neg"]["aj"]  # + reactions["oxygen"]["neg"]["aj"]
            self.rhs = {delta_phi: 1 / param.C_dl_n * (i_cell / param.l_n - sum_j)}
            # self.algebraic = {delta_phi: (i_cell / param.l_n - sum_j)}
            self.initial_conditions = {delta_phi: param.U_n(param.c_e_init)}
            self.variables = {}
        elif domain == ["positive electrode"]:
            i_e = i_cell * (1 - x_p) / param.l_p
            # sum_j = sum(rxn["pos"]["aj"] for rxn in reactions.values())
            sum_j = reactions["main"]["pos"]["aj"] + reactions["oxygen"]["pos"]["aj"]
            self.rhs = {delta_phi: 1 / param.C_dl_p * (-i_cell / param.l_p - sum_j)}
            # self.algebraic = {delta_phi: (-i_cell / param.l_p - sum_j)}
            self.initial_conditions = {delta_phi: param.U_p(param.c_e_init)}
            self.variables = {}
        else:
