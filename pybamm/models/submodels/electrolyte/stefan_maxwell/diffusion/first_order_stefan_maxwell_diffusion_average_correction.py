...
# (first-order with average correction)
#
import pybamm
from .base_stefan_maxwell_diffusion import BaseModel


class FirstOrderAverageCorrection(BaseModel):
    ...
    asymptotic expansion). Additional average correction for validity with side
    reactions and in 2D
    ...
    **Extends:** :class:`pybamm.electrolyte.stefan_maxwell.diffusion.BaseModel`
    ...
    def get_fundamental_variables(self):
        A_e = pybamm.Variable(
            "Electrolyte concentration correction constant", domain="current collector"
        )

        variables = {"Electrolyte concentration correction constant": A_e}
    ...
    def get_coupled_variables(self, variables):
        ...
        A_e = variables["Electrolyte concentration correction constant"]
        ...
        variables["A_e"] = -(
            eps_n_0 * c_e_n_1_av + eps_s_0 * c_e_s_1_av + eps_p_0 * c_e_p_1_av
        ...
        variables.update(
            {
                ...
                + A_e,
                "X-averaged first-order separator concentration": c_e_s_1_av + A_e,
    ...
    def set_algebraic(self, variables):
        ...
        c_e_n_1_av = variables[
            "X-averaged first-order negative electrolyte concentration minus A_e"
        ]
        c_e_s_1_av = variables[
            "X-averaged first-order separator concentration minus A_e"
        ]
        c_e_p_1_av = variables[
            "X-averaged first-order positive electrolyte concentration minus A_e"
        ]
        A_e = variables["Electrolyte concentration correction constant"]

        self.algebraic = {
            A_e: A_e
            # - variables["A_e top"] / (l_n * eps_n_0 + l_s * eps_s_0 + l_p * eps_p_0)
            + (eps_n_0 * c_e_n_1_av + eps_s_0 * c_e_s_1_av + eps_p_0 * c_e_p_1_av)
            / (l_n * eps_n_0 + l_s * eps_s_0 + l_p * eps_p_0)
        }
    ...
    def set_initial_conditions(self, variables):
        ...
        c_e_s_1_av = variables["X-averaged first-order separator concentration"]
        c_e_p_1_av = variables[
            "X-averaged first-order positive electrolyte concentration"
        ]
        A_e = variables["Electrolyte concentration correction constant"]

        self.initial_conditions = {A_e: pybamm.Scalar(0)}
