    ...
    **Extends:** :class:`pybamm.interface.BaseInterface`
    ...
    def get_coupled_variables(self, variables):
        ...
        if self.domain == "Negative":
            if self.half_cell:
                R_sei = self.phase_param.R_sei  # Mark Jason - param to phase_param?
                L_sei = variables[f"Total {phase_name}SEI thickness"]  # on interface; Mark Jason - {phase_name}?
                eta_sei = -j_tot_av * L_sei * R_sei
            elif self.options["SEI film resistance"] == "average":
                R_sei = self.phase_param.R_sei
                L_sei_av = variables[f"X-averaged total {phase_name}SEI thickness"] # Mark Jason - {phase_name}?
