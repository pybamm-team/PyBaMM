...
class StandardParameters(object):
    ...
    # Electrical
    voltage_low_cut = (
        voltage_low_cut_dimensional - (U_p_ref - U_n_ref)
    ) / potential_scale
    voltage_high_cut = (
        voltage_high_cut_dimensional - (U_p_ref - U_n_ref)
    ) / potential_scale

    def __init__(self, capacity_limiting_concentration):
        if capacity_limiting_concentration == "electrolyte":
            capacity_limiting_concentration = self.c_e_typ
        elif capacity_limiting_concentration == "negative particle":
            capacity_limiting_concentration = self.c_n_max
        elif capacity_limiting_concentration == "positive particle":
            capacity_limiting_concentration = self.c_p_max
        # diffusional C-rate
        self.tau_discharge = (
            self.F * capacity_limiting_concentration * self.L_x / self.i_typ
        )
        # diffusional C-rate in electrolyte
        self.C_e = self.tau_diffusion_e / self.tau_discharge
        # Electrochemical Reactions
        self.gamma_dl_n = (
            self.C_dl
            * self.potential_scale
            / self.interfacial_current_scale_n
            / self.tau_discharge
        )
        self.gamma_dl_p = (
            self.C_dl
            * self.potential_scale
            / self.interfacial_current_scale_p
            / self.tau_discharge
        )
