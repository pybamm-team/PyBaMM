...
def run_investigation(config_name, var_pts, tol):
    ...
    def get_submesh(model_obj):
        ...
        s_mesh["negative particle"] = pybamm.MeshGenerator(
            pybamm.Exponential1DSubMesh, submesh_params={"side": "right"}
        )
        s_mesh["positive particle"] = pybamm.MeshGenerator(
            pybamm.Exponential1DSubMesh, submesh_params={"side": "right"}
        )
        return s_mesh

    print(
        f"  Cycling for {CYCLES_TO_RUN} cycles (in {NUM_CHUNKS} chunks of {CYCLES_PER_CHUNK})..."
    )
    t0 = time.time()
    ...
    rest_submesh["negative particle"] = pybamm.MeshGenerator(
        pybamm.Exponential1DSubMesh, submesh_params={"side": side}
    )
    rest_submesh["positive particle"] = pybamm.MeshGenerator(
        pybamm.Exponential1DSubMesh, submesh_params={"side": side}
    )
    ...
    if mode == "run":
        ...
        for cfg in ["standard", "refined"]:
            ...
            if os.path.exists(fname):
                ...
                if cn_avg.ndim > 1:
                    cn_avg = np.mean(cn_avg, axis=0)

                plt.plot(
                    d["time"], cn_surf, label=f"{d['config']} (Surf)", linestyle="-"
                )
                plt.plot(
                    d["time"], cn_avg, label=f"{d['config']} (Vol-Avg)", linestyle="--"
                )
        plt.xlabel("Rest Time [h]")
        ...
        for cfg in ["standard", "refined"]:
            ...
            if os.path.exists(fname):
                ...
                if cp_avg.ndim > 1:
                    cp_avg = np.mean(cp_avg, axis=0)

                plt.plot(
                    d["time"], cp_surf, label=f"{d['config']} (Surf)", linestyle="-"
                )
                plt.plot(
                    d["time"], cp_avg, label=f"{d['config']} (Vol-Avg)", linestyle="--"
                )
        plt.xlabel("Rest Time [h]")
        plt.ylabel("Conecntration [mol.m-3]")
            ...
            if "spatial" in d:
                ...
                if "cn_profiles" in s:
                    ...
                    for i, (prof, t_val) in enumerate(zip(profiles, times)):
                        label_t = (
                            f"{t_val * 3600:.1f}s" if t_val < 0.1 else f"{t_val:.2f}h"
                        )
                        if i == 0:
                            label_t = "Start (0s)"
                        if i == len(profiles) - 1:
                            label_t = f"End ({t_val:.1f}h)"
                        ...
                        plt.plot(
                            ...
                            label=label_t,
                            color=colors[i],
                            linewidth=2 if i in [0, len(profiles) - 1] else 1.5,
                        )
                else:
        ...
        plt.title(
            f"Anode Particle Relaxation ({target_cfg.capitalize()} Mesh)", fontsize=16
        )
