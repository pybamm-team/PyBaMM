...
itg_nogrid = integrator(
    "F", "cvodes", {"t": t, "x": x, "ode": ode, "p": casadi.vertcat(p, tlims)}
)
...
start = time.time()
itg_grid_auto(x0=1, p=p_eval)
print("value:", time.time() - start)

# Fails
rep_p = repmat(p, 1, len(t_eval))
jac_grid_auto = Function("j", [p], [itg_grid_auto(x0=x0, p=p * DM.ones(100, 1))["xf"]])
jac_grid_auto(1)
