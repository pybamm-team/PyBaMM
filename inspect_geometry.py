...
def get_geo_params(param_set_name):
    try:
        ...
        c_init = params.get(
            "Initial concentration in negative electrode [mol.m-3]", "N/A"
        )
        c_max = params.get(
            "Maximum concentration in negative electrode [mol.m-3]", "N/A"
        )
...
print(
    f"{'Parameter Set':<15} | {'Vol. Frac':<10} | {'Radius [um]':<12} | {'Thick [um]':<10} | {'C_max':<12}"
)
print("-" * 75)
