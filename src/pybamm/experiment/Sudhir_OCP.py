...
from pybamm.models.submodels.interface.open_circuit_potential.RK_Polynomial_OCP import (
    RK_Open_Circuit_Potential,
)
...
def run_temp():
    ...
    model.submodels["positive primary open-circuit potential"] = (
        RK_Open_Circuit_Potential(
            model.param, "positive", "lithium-ion main", model.options, "primary"
        )
    )
    model.build_model()
    parameter_values = pybamm.ParameterValues("Chen2020")
    parameter_values.update(
        {"Positive electrode RK_OCP [V]": rk_polynomial}, check_already_exists=False
    )
