...
def newton_raphson(func, x0, threshold=1e-6, in_backward_hood=False):
    ...
    while error > threshold and n_iter < 1000:
        ...
        )  # add grad track here for the sake of autograd.functional.jacobian
        f_now = g(x_now)
        ...
        x_new[1] = torch.min(
            torch.tensor([1.0 - _eps, x_new[1]])
        )  # +- 1e-6 is for the sake of torch.log. We don't want log function to blow up at x=0!
...
class FixedPointOperationForwardPass(nn.Module):
    ...
    def forward(self, x):
        ...
        while dcommon_tangent > 1e-4 and n_iter_ct < 300:
            ...
            common_tangent_new = (
                self.G(x_alpha_now, self.params_list, self.T)
                - self.G(x_beta_now, self.params_list, self.T)
            ) / (x_alpha_now - x_beta_now)
