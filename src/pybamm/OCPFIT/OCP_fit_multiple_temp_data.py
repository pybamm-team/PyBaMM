...
def train_multiple_OCVs_and_dS_dx(
    ...
    T_dsdx,
    number_of_Omegas,
    number_of_S_config_excess_omegas,
    order_of_Theta_LiHM_polynomial_expansion,
    polynomial_style,
    learning_rate,
    learning_rate_other_than_H_mix_excess,
    epoch_optimize_params_other_than_H_mix_only_after_this_epoch,
    alpha_dsdx,
    ...
    records_y_lims,
    n_list,
    pretrained_value_of_S_config_excess_omegas,
    pretrained_value_of_Theta_LiHM_polynomial_expasion,
    pretrained_value_of_ThetaHM,
    ...
    x_measured, dsdx_measured = (
        _convert_JMCA_Tdsdx_data_to_dsdx(datafile_dsdx_name, T_dsdx)
        if datafile_dsdx_name
        else (None, None)
    )

    params_list = [
        nn.Parameter(
            torch.tensor(np.random.uniform(*Omegas_rand_range), dtype=torch.float32)
        )
        for _ in range(number_of_Omegas)
    ]
    G0 = nn.Parameter(
        torch.tensor(np.random.uniform(*G0_rand_range), dtype=torch.float32)
    )
    ...
    S_config_params_list = (
        [
            nn.Parameter(
                torch.tensor(
                    pretrained_value_of_S_config_excess_omegas[i], dtype=torch.float32
                )
            )
            for i in range(number_of_S_config_excess_omegas)
        ]
        if pretrained_value_of_S_config_excess_omegas
        else [
    ...
    Theta_LiHM = (
        ...
        if pretrained_value_of_Theta_LiHM_polynomial_expasion
        else [
            nn.Parameter(
                torch.tensor(np.random.uniform(-1, 1) / 100, dtype=torch.float32)
            )
    ...
    Theta_HM = (
        nn.Parameter(
            torch.tensor(pretrained_value_of_ThetaHM / 100, dtype=torch.float32)
        )
        if pretrained_value_of_ThetaHM
        else nn.Parameter(torch.tensor(3.0, dtype=torch.float32))
    )
    Theta_Li = (
        nn.Parameter(
            torch.tensor(pretrained_value_of_ThetaLi / 100, dtype=torch.float32)
        )
        if pretrained_value_of_ThetaLi
        else nn.Parameter(torch.tensor(3.0, dtype=torch.float32))
    )
