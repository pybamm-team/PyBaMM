...
from energy import (
    ...
    convex_hull,
    CommonTangent,
    calculate_S_config_total,
    calculate_S_vib_total,
)
...
# loss function
def collocation_loss_all_pts(
    mu,
    x,
    phase_boundarys_fixed_point,
    GibbsFunction,
    params_list,
    alpha_miscibility,
    T=300,
    ...
    for i in range(0, len(x)):
        ...
        if is_inside == False:
            ...
            mu_pred_now = autograd.grad(outputs=g_now, inputs=x_now, create_graph=True)[
                0
            ]
            ...
            ct_pred = (
                GibbsFunction(x_alpha, params_list, T)
                - GibbsFunction(x_beta, params_list, T)
            ) / (x_alpha - x_beta)
            ...
            if x_alpha > x_beta:
                print(
                    "Error in phase equilibrium boundary, x_left %.4f larger than x_right %.4f. If Hessian loss is not 0, it's fine. Otherwise check code carefully!"
                    % (x_alpha, x_beta)
                )
                x_alpha = 99999.9
                x_beta = -99999.9
            if torch.isnan(ct_pred):
                ...
                loss_ = (
                    loss_ + alpha_miscibility * ((ct_pred - mu_now) / (8.314 * T)) ** 2
                )
...
def calc_loss_entropy(
    x_measured, dsdx_measured, S_config_params_list, n_list, Theta_E_list, T_dsdx=320
):
    ...
    for i in range(0, len(x_measured)):
        ...
        s_vib_tot = calculate_S_vib_total(
            x, n_list, Theta_E_list, T=T_dsdx, Theta_Li_scaled_100_times=True
        )
...
def train_multiple_OCVs_and_dS_dx(
    ...
    if datafile_dsdx_name != None:
        x_measured, dsdx_measured = _convert_JMCA_Tdsdx_data_to_dsdx(
            datafile_name=datafile_dsdx_name, T=T_dsdx
        )
    ...
    if number_of_Omegas <= 10:
        for _ in range(0, number_of_Omegas):
            ...
            params_list.append(
                nn.Parameter(
                    torch.from_numpy(np.array([Omegai_start], dtype="float32"))
                )
            )
    else:
        for _ in range(0, 10):
    ...
    G0_start = np.random.randint(
        G0_rand_range[0], G0_rand_range[1]
    )  # G0 is the pure substance gibbs free energy
    ...
    if pretrained_value_of_S_config_excess_omegas == None:
        for _ in range(0, number_of_S_config_excess_omegas):
            S_config_params_list.append(
                nn.Parameter(
                    torch.from_numpy(
                        np.array([np.random.randint(-100, 100) * 0.01], dtype="float32")
                    )
                )
            )
    else:
        for _ in range(0, number_of_S_config_excess_omegas):
            S_config_params_list.append(
                nn.Parameter(
                    torch.from_numpy(
                        np.array(
                            [pretrained_value_of_S_config_excess_omegas[_]],
                            dtype="float32",
                        )
                    )
                )
    ...
    if pretrained_value_of_Theta_LiHM_polynomial_expasion == None:
        coeff_now = nn.Parameter(
            torch.from_numpy(np.array([200.0 / 100], dtype="float32"))
        )  # Theta_Li is scaled by 100 time
        Theta_LiHM.append(coeff_now)
        for i in range(1, order_of_Theta_LiHM_polynomial_expansion + 1):
            coeff_now = nn.Parameter(
                torch.from_numpy(
                    np.array([(np.random.random() * 2 - 1) / 100], dtype="float32")
                )
            )  # Theta_Li is scaled by 100 time
            Theta_LiHM.append(coeff_now)
    else:
        for i in range(0, order_of_Theta_LiHM_polynomial_expansion + 1):
            coeff_now = nn.Parameter(
                torch.from_numpy(
                    np.array(
                        [pretrained_value_of_Theta_LiHM_polynomial_expasion[i] / 100],
                        dtype="float32",
                    )
                )
            )  # Theta_Li is scaled by 100 time
    ...
    if pretrained_value_of_ThetaHM == None:
        Theta_HM = nn.Parameter(
            torch.from_numpy(np.array([300.0 / 100], dtype="float32"))
        )  # Theta_Li is scaled by 100 time
    else:
        Theta_HM = nn.Parameter(
            torch.from_numpy(
                np.array([pretrained_value_of_ThetaHM / 100], dtype="float32")
            )
        )  # Theta_Li is scaled by 100 time
    if pretrained_value_of_ThetaLi == None:
        ...
        Theta_Li = nn.Parameter(
            torch.from_numpy(
                np.array([pretrained_value_of_ThetaLi / 100], dtype="float32")
            )
        )  # Theta_Li is scaled by 100 time
    ...
    # init optimizer
    optimizer_before_critical_epochs = optim.Adam(
        [{"params": params_list, "lr": learning_rate}]
    )
    optimizer_after_critical_epochs = optim.Adam(
        [
            {"params": params_list, "lr": learning_rate},
            {
                "params": params_list_other_than_H_mix_excess,
                "lr": learning_rate_other_than_H_mix_excess,
            },
        ]
    )
    ...
    while loss > loss_threshold and epoch < total_training_epochs:
        ...
        # for datafile 1
        phase_boundary_fixed_point = _get_phase_boundaries(
            GibbsFE, params_list, S_config_params_list, n_list, Theta_E_list, T1
        )
        loss_collocation_1 = collocation_loss_all_pts(
            mu1,
            x1,
            phase_boundary_fixed_point,
            GibbsFE,
            [params_list, S_config_params_list, n_list, Theta_E_list],
            1.0,
            T=T1,
        )
        ...
        # datafile 2
        if datafile2_name != None:
            phase_boundary_fixed_point2 = _get_phase_boundaries(
                GibbsFE, params_list, S_config_params_list, n_list, Theta_E_list, T2
            )
            loss_collocation_2 = collocation_loss_all_pts(
                mu2,
                x2,
                phase_boundary_fixed_point2,
                GibbsFE,
                [params_list, S_config_params_list, n_list, Theta_E_list],
                1.0,
                T=T2,
        ...
        # datafile 3
        if datafile3_name != None:
            phase_boundary_fixed_point3 = _get_phase_boundaries(
                GibbsFE, params_list, S_config_params_list, n_list, Theta_E_list, T3
            )
            loss_collocation_3 = collocation_loss_all_pts(
                mu3,
                x3,
                phase_boundary_fixed_point3,
                GibbsFE,
                [params_list, S_config_params_list, n_list, Theta_E_list],
                1.0,
                T=T3,
        ...
        if datafile_dsdx_name != None:
            (
                loss_entropy,
                _loss_dsdx,
                _loss_s_config_leq_0,
                _loss_s_config_geq_upper_bound,
            ) = calc_loss_entropy(
                x_measured,
                dsdx_measured,
                S_config_params_list,
        ...
        # print output
        output_txt = (
            "Epoch %3d  Loss %.4f  OCV1 %.4f  OCV2 %.4f  OCV3 %.4f  dSdx_col %.4f s<0 %.4f s>smax%.4f     "
            % (
                epoch,
                loss,
                loss_collocation_1,
        ...
        for i in range(0, len(S_config_params_list)):
            output_txt = output_txt + "omega%d %.4f " % (
                i,
                S_config_params_list[i].item(),
            )
        # Theta_E params
        for i in range(0, len(Theta_E_list[0])):
            output_txt = output_txt + "ThetaE0_%d %.4f " % (
                i,
                Theta_E_list[0][i].item() * 100,
            )
        for i in range(1, len(Theta_E_list)):
            output_txt = output_txt + "ThetaE%d %.4f " % (
                i,
                Theta_E_list[i].item() * 100,
            )
        ...
        if epoch % 100 == 0:
            ...
            for i in range(0, len(x1)):
                ...
                mu_pred_now = autograd.grad(
                    outputs=g_now, inputs=x_now, create_graph=True
                )[0]
            ...
            plt.plot(
                SOC, U_pred_before_ct, "k--", label="Prediction Before CT Construction"
            )
            ...
            plt.plot(
                SOC, U_pred_after_ct, "r-", label="Prediction After CT Construction"
            )
...
def write_ocv_functions(params_list, T=300, outpyfile_name="fitted_ocv_functions.py"):
    ...
    if phase_boundarys_init != []:
        ...
        for phase_boundary_init in phase_boundarys_init:
            common_tangent = CommonTangent(
                GibbsFE, params_list, T=T
            )  # init common tangent model
    ...
    if len(phase_boundary_fixed_point) > 0:
        print(
            "Found %d phase coexistence region(s):" % (len(phase_boundary_fixed_point))
        )
        for i in range(0, len(phase_boundary_fixed_point)):
            ...
            ct_now = (
                GibbsFE(x_alpha, params_list, T=T) - GibbsFE(x_beta, params_list, T=T)
            ) / (x_alpha - x_beta)
            cts.append(ct_now)
            print(
                "From x=%.16f to x=%.16f, mu_coex=%.16f"
                % (
                    phase_boundary_fixed_point[i][0],
                    phase_boundary_fixed_point[i][1],
                    ct_now,
                )
            )
    else:
    ...
    with open(outpyfile_name, "w") as fout:
        ...
        if isinstance(params_list[0], list) == True and len(params_list) == 4:
            ...
            fout.write(
                "    G0 = %.6f # G0 is the pure substance gibbs free energy \n"
        ...
        if len(phase_boundary_fixed_point) > 0:
            for i in range(0, len(phase_boundary_fixed_point)):
                ...
                fout.write(
                    "    is_outside_miscibility_gap_%d = (sto<x_alpha_%d) + (sto>x_beta_%d)\n"
                    % (i, i, i)
                )
            ...
            fout.write("    \n")
            fout.write(
                "    mu_outside = G0 + 8.314*%.4f*log((sto+_eps)/(1-sto+_eps))\n" % (T)
            )
            if isinstance(params_list[0], list) == True and len(params_list) == 4:
                # this means the first one is excess enthalpy free energy params,
                # the second one is excess config entropy params
                ...
                fout.write(
                    "    _t = 1 - 2 * sto  # Transform x to (1 - 2x) for legendre expansion\n"
                )
                fout.write(
                    "    Pn_values = legendre_poly_recurrence(_t,len(ThetaEs[0])-1)\n"
                )
                fout.write(
                    "    Pn_derivatives_values = legendre_derivative_poly_recurrence(_t, len(ThetaEs[0])-1)  # Compute Legendre polynomials up to degree len(coeffs) - 1\n"
                )
                ...
                fout.write(
                    "        Theta_LiHM = Theta_LiHM + ThetaEs[0][i]*Pn_values[i]\n"
                )
                fout.write(
                    "        Theta_LiHM_derivative = Theta_LiHM_derivative + (-2)*ThetaEs[0][i]*Pn_derivatives_values[i]\n"
                )
                fout.write("    t_Theta_LiHM = -Theta_LiHM*100/%.4f\n" % (T))
                fout.write(
                    "    t_Theta_LiHM_derivative = Theta_LiHM_derivative*100/%.4f  # NOTE that there is no negative sign here\n"
                    % (T)
                )
                fout.write(
                    "    mu_outside = mu_outside - %.4f*(-3)*(ns[2])*8.314*( log(1.0 - exp(t_Theta_LiHM)) + t_Theta_LiHM*1.0/(exp(-t_Theta_LiHM)-1) ) \n"
                    % (T)
                )
                fout.write(
                    "    mu_outside = mu_outside - %.4f*(-3)*(1.0*ns[1]+sto*ns[2])*8.314*( 1/(1-exp(t_Theta_LiHM))* exp(t_Theta_LiHM)*t_Theta_LiHM_derivative  - t_Theta_LiHM_derivative*1/(exp(-t_Theta_LiHM) -1)   -t_Theta_LiHM * exp(-t_Theta_LiHM)/(exp(-t_Theta_LiHM) -1)**2 *t_Theta_LiHM_derivative  ) \n"
                    % (T)
                )
                ...
                fout.write(
                    "    mu_outside = mu_outside +  %.4f*(-3)*ns[2]*8.314*( log(1.0 - exp(t)) + t*1.0/(exp(-t)-1) ) \n"
                    % (T)
                )
                ...
                fout.write(
                    "    Theta_LiHM_derivative_real = Theta_LiHM_derivative*100\n"
                )
                fout.write(
                    "    mu_outside = mu_outside +  3*(ns[2])*8.314*( 1/2*Theta_LiHM_real + Theta_LiHM_real/(exp(Theta_LiHM_real/%.4f)-1) ) \n"
                    % (T)
                )
                fout.write(
                    "    mu_outside = mu_outside +  3*(1.0*ns[1]+sto*ns[2])*8.314*( 0.5*Theta_LiHM_derivative_real + Theta_LiHM_derivative_real/(exp(Theta_LiHM_real/%.4f) -1) - Theta_LiHM_real/(exp(Theta_LiHM_real/%.4f) -1)**2 *  exp(Theta_LiHM_real/%.4f) * Theta_LiHM_derivative_real/%.4f ) \n"
                    % (T, T, T, T)
                )
                ## mu does not depend on HM
                # fout.write("    # Theta_HM \n")
                ...
                fout.write(
                    "    mu_outside = mu_outside - 3*ns[2]*8.314*( 1/2*Theta_Li_real + Theta_Li_real/(exp(Theta_Li_real/%.4f)-1) ) \n"
                    % (T)
                )
                ...
                fout.write(
                    "        mu_outside = mu_outside + 8.314*%.4f*log((sto+_eps)/(1-sto+_eps))*(omegas[i]*Pn_values[i]) -2.0*8.314*%.4f*(sto*log(sto) + (1-sto)*log(1-sto))*(omegas[i]*Pn_derivatives_values[i]) \n"
                    % (T, T)
                )
            # write excess G part
            fout.write(
            ...
            if isinstance(params_list[0], list) == True and len(params_list) == 4:
                ...
                fout.write(
                    "    mu = mu - %.4f*(-3)*(ns[2])*8.314*( log(1.0 - exp(t_Theta_LiHM)) + t_Theta_LiHM*1.0/(exp(-t_Theta_LiHM)-1) ) \n"
                    % (T)
                )
                fout.write(
                    "    mu = mu - %.4f*(-3)*(1.0*ns[1]+sto*ns[2])*8.314*( 1/(1-exp(t_Theta_LiHM))* exp(t_Theta_LiHM)*t_Theta_LiHM_derivative  - t_Theta_LiHM_derivative*1/(exp(-t_Theta_LiHM) -1)   -t_Theta_LiHM * exp(-t_Theta_LiHM)/(exp(-t_Theta_LiHM) -1)**2 *t_Theta_LiHM_derivative  ) \n"
                    % (T)
                )
                ## mu does not depend on HM,
                # fout.write("    # Theta_HM \n")
                ...
                fout.write("    t = -ThetaEs[2]*100/%.4f\n" % (T))
                fout.write(
                    "    mu = mu -  %.4f*(-3)*ns[2]*8.314*2*sto*( log(1.0 - exp(t)) + t*1.0/(exp(-t)-1) ) \n"
                    % (T)
                )
                ## write H_vib contribution
                fout.write("    ## H_vib\n")
                ...
                fout.write(
                    "    mu = mu +  3*(ns[2])*8.314*( 1/2*Theta_LiHM_real + Theta_LiHM_real/(exp(Theta_LiHM_real/%.4f)-1) ) \n"
                    % (T)
                )
                fout.write(
                    "    mu = mu +  3*(1.0*ns[1]+sto*ns[2])*8.314*( 0.5*Theta_LiHM_derivative_real + Theta_LiHM_derivative_real/(exp(Theta_LiHM_real/%.4f) -1) - Theta_LiHM_real/(exp(Theta_LiHM_real/%.4f) -1)**2 *  exp(Theta_LiHM_real/%.4f) * Theta_LiHM_derivative_real/%.4f ) \n"
                    % (T, T, T, T)
                )
                ## mu does not depend on HM
                # fout.write("    # Theta_HM \n")
                ...
                fout.write("    Theta_Li_real = ThetaEs[2]*100\n")
                fout.write(
                    "    mu = mu + 3*ns[2]*8.314*2*sto*( 1/2*Theta_Li_real + Theta_Li_real/(exp(Theta_Li_real/%.4f)-1) ) \n"
                    % (T)
                )
                ## write S_config contribution
                ...
                fout.write("    for i in range(0, len(omegas)):\n")
                fout.write(
                    "        mu = mu + 8.314*%.4f*log((sto+_eps)/(1-sto+_eps))*(omegas[i]*Pn_values[i]) -2.0*8.314*%.4f*(sto*log(sto) + (1-sto)*log(1-sto))*(omegas[i]*Pn_derivatives_values[i]) \n"
                    % (T, T)
                )
            # excess G
            fout.write(
    ...
    print("###################################\n")
    with open(outpyfile_name) as fin:
        lines = fin.readlines()
    for line in lines:
        print(line, end="")
    print("\n\n###################################\n")
    print(
        "Or check %s and fitted_ocv_functions.m (if polynomial style = R-K) for fitted thermodynamically consistent OCV model in PyBaMM & Matlab formats. "
        % (outpyfile_name)
    )
...
def _convert_JMCA_Tdsdx_data_to_dsdx(
    datafile_name="TdS_dx_lithiation_320K_modified.csv", T=320
):
    # read hysterisis data
    working_dir = os.getcwd()
    df = pd.read_csv(
        datafile_name, header=None
...
def _get_phase_boundaries(
    GibbsFE, params_list, S_config_params_list, n_list, Theta_E_list, T
):
    # sample the Gibbs free energy landscape
    sample = sampling(
        GibbsFE,
    ...
    if phase_boundarys_init != []:
        ...
        for phase_boundary_init in phase_boundarys_init:
            common_tangent = CommonTangent(
                GibbsFE, [params_list, S_config_params_list, n_list, Theta_E_list], T=T
            )  # init common tangent model
            phase_boundary_now = phase_boundary_init.requires_grad_()
            phase_boundary_fixed_point_now = common_tangent(phase_boundary_now)
            phase_boundary_fixed_point.append(phase_boundary_fixed_point_now)
