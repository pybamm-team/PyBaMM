...
def GibbsFE_Legendre(x, params_list, T=300):
    ...
    if isinstance(params_list[0], list) == True and len(params_list) == 4:
        ...
        G = (
            x * G0
            + (1 - x) * 0.0
            - T * (-8.314) * (x * torch.log(x) + (1 - x) * torch.log(1 - x))
        )
        t = 1 - 2 * x  # Transform x to (1 - 2x) for legendre expansion
        # excess configurational entropy
        Pn_values = legendre_poly_recurrence(
            t, len(entropy_params_list) - 1
        ...
        for i in range(0, len(entropy_params_list)):
            G = G - T * (-8.314) * (x * torch.log(x) + (1 - x) * torch.log(1 - x)) * (
                entropy_params_list[i] * Pn_values[i]
            )
        # vibrational entropy
        ...
        # excess enthalpy
        Pn_values = legendre_poly_recurrence(
            t, len(energy_params_list) - 2
        )  # Compute Legendre polynomials up to degree len(coeffs) - 1 # don't need to get Pn(G0)
...
def calculate_S_vib_total(
    x, n_list, Theta_E_list, T=320, Theta_Li_scaled_100_times=True
):
    """
...
def _H_vib_single_Einstein_model(x, Theta_Li, T=320, Theta_Li_scaled_100_times=True):
    ...
    H_vib = (
        3
        * 8.314
        * (1 / 2 * Theta_Li_real + Theta_Li_real / (torch.exp(Theta_Li_real / T) - 1))
    )
    return H_vib


def calculate_H_vib_total(
    x, n_list, Theta_E_list, T=300, Theta_Li_scaled_100_times=True
):
    """
    Corresponding H_vib given S_vib, we need to satisfy dH/dT = TdS/dT
    Weighted average of several Einstein models, the number of Einstein models is len(n_list) = len(Theta_E_list)
    ...
    ## LixHM: there is 1 mole of HM, and x mole of Li
    H_vib = H_vib + (1.0 * n_list[1] + x * n_list[2]) * _H_vib_single_Einstein_model(
        x, Theta_E_list[0], T=T, Theta_Li_scaled_100_times=Theta_Li_scaled_100_times
    )
    ## HM: there is 1 mole of HM
    H_vib = H_vib - (1.0 * n_list[1]) * _H_vib_single_Einstein_model(
        x, Theta_E_list[1], T=T, Theta_Li_scaled_100_times=Theta_Li_scaled_100_times
    )
    ## Li: there is x mole of Li
    H_vib = H_vib - (x * n_list[2]) * _H_vib_single_Einstein_model(
        x, Theta_E_list[2], T=T, Theta_Li_scaled_100_times=Theta_Li_scaled_100_times
    )
    return H_vib
...
def sampling(GibbsFunction, params_list, T, sampling_id, ngrid=99, requires_grad=False):
    ...
    x = np.concatenate(
        (
            np.array([_eps]),
            np.linspace(1 / (ngrid + 1), 1 - 1 / (ngrid + 1), ngrid),
            np.array([1.0 - _eps]),
        )
    )
    ...
    sample = torch.tensor(
        [
            [x[i], GibbsFunction(x[i], params_list, T), sampling_id]
...
def convex_hull(sample, ngrid=99, tolerance=_eps):
    ...
    while new_base_length != current_base_length:
        ...
        for i in range(
            len(base_working)
        ):  # len(base_working) = 1 at first, but after iterations on n, the length of this list will be longer
            ...
            t = base_working[i][
                1
            ]  # h is the sample point at left side, t is the sample point at right side
            _n = torch.matmul(
                torch.from_numpy(np.array([[0.0, -1.0], [1.0, 0.0]]).astype("float32")),
                torch.reshape((t[0:2] - h[0:2]), (2, 1)),
            )
            # limit to those having x value between the x value of h and t
            left_id = (
                torch.argmin(torch.abs(sample[:, 0] - h[0])) + 1
            )  # limiting the searching range within h and t
            right_id = torch.argmin(torch.abs(sample[:, 0] - t[0]))
            if (
                left_id == right_id
            ):  # it means this piece of convex hull is the shortest piece possible
                ...
                if (
                    dists.shape == torch.Size([])
                ):  # in case that there is only 1 item in dists, .squeeze wil squeeze ALL dimension and make dists a 0-dim tensor
                ...
                if len(outer):
                    pivot = sample_current[
                        torch.argmin(dists)
                    ]  # the furthest node below the hyperplane defined hy t[column]-h[column]
                    ...
                    while z <= len(base) - 1:
                        ...
                        diff = torch.max(
                            torch.abs(
                                torch.cat((base[z][0], base[z][1]))
                                - torch.cat((base_working[i][0], base_working[i][1]))
                            )
                        )
                        if diff < tolerance:
                            # remove this plane
    ...
    for i in range(0, len(base)):
        ...
        if convex_hull_piece_now[1][0] - convex_hull_piece_now[0][0] > delta_x:
            miscibility_gap_x_left_and_right.append(
                torch.tensor([convex_hull_piece_now[0][0], convex_hull_piece_now[1][0]])
            )
            miscibility_gap_phase_left_and_right.append(
                torch.tensor([convex_hull_piece_now[0][2], convex_hull_piece_now[1][2]])
            )
    ...
    for _ in range(0, len(index)):
        miscibility_gap_x_left_and_right_sorted.append(
            miscibility_gap_x_left_and_right[_]
        )
        miscibility_gap_phase_left_and_right_sorted.append(
            miscibility_gap_phase_left_and_right[_]
        )
    return (
        miscibility_gap_x_left_and_right_sorted,
        miscibility_gap_phase_left_and_right_sorted,
...
class CommonTangent(nn.Module):
    ...
    def __init__(self, G, params_list, T=300):
        ...
        self.f_forward = FixedPointOperationForwardPass(
            G, params_list, T
        )  # define forward operation here
        self.f = FixedPointOperation(
            G, params_list, T
        )  # define backward operation here
    ...
    def forward(self, x, **kwargs):
        ...
        # Forward pass
        x_star = self.solver(
            self.f, x, threshold=self.f_thres
        )  # use newton-raphson to get the fixed point
        if (
            torch.any(torch.isnan(x_star)) == True
        ):  # in case that the previous one doesn't work
            print(
                "Fixpoint solver failed at T = %d. Use traditional approach instead"
                % (self.T)
            )
        ...
        )  # go through the process again to get derivative
