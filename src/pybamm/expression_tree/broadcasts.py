...
class Broadcast(pybamm.SpatialOperator):
    ...
    <https://github.com/pybamm-team/PyBaMM/blob/main/docs/source/examples/notebooks/expression_tree/broadcasts.ipynb>`_
    ...
    def check_and_set_domains(self, child: pybamm.Symbol, broadcast_domain: list[str]):
        ...
        if child.domain == []:
            ...
            raise pybamm.DomainError(
                ...
                (core or shell) or particle size domains"""
            )
        elif child.domain[0] in [
...
class SecondaryBroadcast(Broadcast):
    ...
    def check_and_set_domains(self, child: pybamm.Symbol, broadcast_domain: list[str]):
        ...
        if child.domain[0] in [
            ...
            raise pybamm.DomainError(
                """Secondary broadcast from particle (core or shell) domain must
                be to particle-size, electrode, separator, or
