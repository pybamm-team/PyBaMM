...
def is_matrix_x(expr: Symbol, x: int):
    ...
    if is_constant(expr):
        ...
        ) or (isinstance(result, npt.NDArray) and np.all(result == x))
    else:
        return False
        ...
        if result is not None:
            if (
                ...
                if result.ndim == 1 or result.shape[1] == 1:
                    ...
                    if isinstance(result, npt.NDArray) and np.all(result == 0):
