...
def is_matrix_x(expr: Symbol, x: int):
    ...
    if is_constant(expr):
        ...
        ) or (isinstance(result, np.typing.NDArray) and np.all(result == x))
    else:
        return False
    ...
    if symbol.is_constant():
        ...
        if result is not None:
            if (
                ...
                or (isinstance(result, np.typing.NDArray) and result.ndim == 0)
                or isinstance(result, np.bool_)
            ):
                # type-narrow for Scalar
                new_result = cast(float, result)
                return pybamm.Scalar(new_result)
            elif isinstance(result, np.typing.NDArray) or issparse(result):
                if result.ndim == 1 or result.shape[1] == 1:
                    ...
                    if isinstance(result, np.typing.NDArray) and np.all(result == 0):
