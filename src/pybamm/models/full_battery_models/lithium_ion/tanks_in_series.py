...
class TanksInSeries(BaseModel):
    ...
    def __init__(self, name="Tanks-in-Series Model"):
        ...
        c_e_av_n = pybamm.Variable(
            "X-averaged negative electrolyte concentration [mol.m-3]"
        )
        c_e_av_s = pybamm.Variable(
            "X-averaged separator electrolyte concentration [mol.m-3]"
        )
        c_e_av_p = pybamm.Variable(
        ...
        phi_e_av_n = pybamm.Variable("X-averaged negative electrolyte potential [V]")
        phi_e_av_s = pybamm.Variable("X-averaged separator electrolyte potential [V]")
        phi_e_av_p = pybamm.Variable("X-averaged positive electrolyte potential [V]")
        ...
        j0_n = param.n.prim.j0(c_e_av_n, c_surf_ave_n, T)
        eta_n = phi_s_n - phi_e_av_n - param.n.prim.U(sto_surf_n, T)
        ...
        j0_p = param.p.prim.j0(c_e_av_p, c_surf_ave_p, T)
        eta_p = phi_s_p - phi_e_av_p - param.p.prim.U(sto_surf_p, T)
        ...
        c_12 = (
            eps_p**param.p.b_e / param.p.L * c_e_av_p
            + eps_sep**param.s.b_e / param.s.L * c_e_av_s
        ) / (eps_p**param.p.b_e / param.p.L + eps_sep**param.s.b_e / param.s.L)
        # 23: boundary between separator and negative electrode
        c_23 = (
            eps_n**param.n.b_e / param.n.L * c_e_av_n
            + eps_sep**param.s.b_e / param.s.L * c_e_av_s
        ...
        self.rhs[c_e_av_n] = (
            ...
            * (c_e_av_n - c_e_av_s)
            / leps23
            / (eps_n * param.n.L)
            - (1 - param.t_plus(c_e_av_n, T)) * iapp / param.F / eps_n / param.n.L
        )
        self.rhs[c_e_av_s] = (
            -2 * param.D_e(c_12, T) * (c_e_av_s - c_e_av_p) / leps12
            + 2 * param.D_e(c_23, T) * (c_e_av_n - c_e_av_s) / leps23
        ) / (eps_sep * param.s.L)
        self.rhs[c_e_av_p] = (
            2
            * param.D_e(c_12, T)
            * (c_e_av_s - c_e_av_p)
            / leps12
            / (eps_p * param.p.L)
            + (1 - param.t_plus(c_e_av_p, T)) * iapp / param.F / eps_p / param.p.L
        )

        self.initial_conditions[c_e_av_n] = param.c_e_init
        self.initial_conditions[c_e_av_s] = param.c_e_init
        self.initial_conditions[c_e_av_p] = param.c_e_init
        ...
        self.algebraic[phi_e_av_p] = (
            -iapp
            - 2 * param.kappa_e(c_12, T) * (phi_e_av_s - phi_e_av_p) / leps12
            + 2 * param.chiRT_over_Fc(c_12, T) * (c_e_av_s - c_e_av_p) / leps12
        )
        self.algebraic[phi_e_av_n] = (
            -iapp
            - 2 * param.kappa_e(c_23, T) * (phi_e_av_n - phi_e_av_s) / leps23
            + 2 * param.chiRT_over_Fc(c_23, T) * (c_e_av_n - c_e_av_s) / leps23
        )
        self.algebraic[phi_e_av_s] = iapp / param.F / a_n / param.n.L + pore_wall_flux_n

        self.initial_conditions[phi_e_av_p] = pybamm.Scalar(0)
        self.initial_conditions[phi_e_av_n] = pybamm.Scalar(0)
        self.initial_conditions[phi_e_av_s] = pybamm.Scalar(0)
        ...
        # To compare the tanks-in-series model with other models,
        # the average variables are broadcast to the x or r domain of relevance.

        # Broadcasting the electrolyte concentration for compatibility with built-in plotting
        c_e_n = pybamm.Variable("Negative electrolyte concentration [mol.m-3]")
        c_e_n = pybamm.PrimaryBroadcast(c_e_av_n, "negative electrode")
        c_e_s = pybamm.Variable("Separator electrolyte concentration [mol.m-3]")
        c_e_s = pybamm.PrimaryBroadcast(c_e_av_s, "separator")
        c_e_p = pybamm.Variable("Positive electrolyte concentration [mol.m-3]")
        c_e_p = pybamm.PrimaryBroadcast(c_e_av_p, "positive electrode")
        c_e = pybamm.concatenation(c_e_n, c_e_s, c_e_p)

        # Electrolyte potential
        # for plotting from average to broadcasted values

        phi_e_n = pybamm.Variable("Negative electrolyte potential [V]")
        phi_e_n = pybamm.PrimaryBroadcast(phi_e_av_n, "negative electrode")
        phi_e_s = pybamm.Variable("Separator electrolyte potential [V]")
        phi_e_s = pybamm.PrimaryBroadcast(phi_e_av_s, "separator")
        phi_e_p = pybamm.Variable("Positive electrolyte potential [V]")
        phi_e_p = pybamm.PrimaryBroadcast(phi_e_av_p, "positive electrode")
        phi_e = pybamm.concatenation(phi_e_n, phi_e_s, phi_e_p)
        #
        # {"Negative electrolyte "
        #     "potential [V]": pybamm.PrimaryBroadcast(
        #         phi_e_av_n, "negative electrode"
        #     )}
        # phi_e = pybamm.concatenation(
        #     {"Negative electrolyte "
        #     "potential [V]": pybamm.PrimaryBroadcast(
        #         phi_e_av_n, "negative electrode"
        #     )},
        ...
        #     "potential [V]": pybamm.PrimaryBroadcast(
        #         phi_e_av_p, "positive electrode"
        #     )},
        #     {"Separator electrolyte "
        #     "potential [V]": pybamm.PrimaryBroadcast(
        #         phi_e_av_s, "separator"
        #     )}
        # )

        self.variables = {
            # 'Negative particle concentration [mol.m-3]': pybamm.FullBroadcast(
            #     c_s_n, 'negative particle', 'negative electrode'
            # ),
            ...
            "concentration [mol.m-3]": pybamm.PrimaryBroadcast(
                c_surf_ave_n, "negative electrode"
            ),
            "Positive particle surface "
            "concentration [mol.m-3]": pybamm.PrimaryBroadcast(
                c_surf_ave_p, "positive electrode"
            ),
            ...
            #     phi_e_av_s, "separator"
            # ),
            # "X-averaged negative electrolyte concentration [mol.m-3]": c_e_av_n,
            # "X-averaged positive electrolyte concentration [mol.m-3]": c_e_av_p,
            # "X-averaged separator electrolyte concentration [mol.m-3]": c_e_av_s,
            ...
            # "X-averaged positive electrolyte potential [V]": phi_e_av_p,
            # "X-averaged negative electrolyte potential [V]": phi_e_av_n,
            # "X-averaged separator electrolyte potential [V]": phi_e_av_s,
