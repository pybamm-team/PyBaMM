...
class MyThermalModel(pybamm.models.submodels.thermal.BaseThermal):
    ...
    def set_rhs(self, variables):
        ...
        dTdt = (1 / (ro * Cp)) * pybamm.div(k * pybamm.grad(T_cylinder)) + Q_heat / (
            V * ro * Cp
        )
    ...
    def set_boundary_conditions(self, variables):
        ...
        self.boundary_conditions = {
            T_cylinder: {"left": (lbc, "Neumann"), "right": (rbc, "Neumann")}
        }
...
param1.update(
    {
        "Positive electrode RK_OCP [V]": my_rk_ocp_positive,
        "Negative electrode RK_OCP [V]": my_rk_ocp_negative,
        "Current function [A]": 7.2,
    },
    check_already_exists=False,
)

param2 = pybamm.ParameterValues(
    {
        "Heat conductivity [W/m2-K]": 0.2,
        "Convection heat transfer coefficient [W/m2-K]": 20,
        "Initial temperature [K]": 300,
        "Cell height [m]": 0.07,
        "Cell radius [m]": 0.0105,
...
R = pybamm.Parameter("Cell radius [m]")
# define geometry
r_cell = pybamm.SpatialVariable("r_cell", domain=["full cell"], coord_sys="cylindrical")
geometry2 = {"full cell": {r_cell: {"min": pybamm.Scalar(0), "max": R}}}
